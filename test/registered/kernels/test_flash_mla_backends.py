...
class TestTouchedPageSplit(CustomTestCase):
    ...
    Ported from upstream sglang #32320. The destination buffer is persistent
    across decode steps, so the masked split is only correct if it (a) copies
    every byte of the data and scale regions of a marked page's sub-pages and
    (b) leaves everything else alone -- the per-sub-page alignment tail and all
    sub-pages of an unmarked source page. A widened copy or a dropped mask
    check violates the persistent-buffer contract, which checking only the
    copied regions would not catch.

    The desk-executable arm of the same logic (real kernels, Triton
    interpreter, CPU) is
