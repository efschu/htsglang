...
def test_the_family_complete_wake_zeroes_both_runners_after_the_static_import():
    """UNIFY S2: one call site for both profiles (the NF one, after the legs,
    target before the H25 draft unpark); the 27B placement intent lives in the
    helper as a guard: the whole-model memset runs only once the weights family
    is COMPLETE, never in a partial wake chunk."""
    src = inspect.getsource(WU.SchedulerWeightUpdaterManager.resume_memory_occupation)
    full = inspect.getsource(WU)
    helper = inspect.getsource(WU.SchedulerWeightUpdaterManager._weg2_zero_local_scratch)
    assert "from sglang.srt.weg2.weight_exchange import zero_local_scratch" in helper
    assert "WEG2-RESUME local-scratch zeroed=%d" in src
    assert "_wake_models = self._weg2_wake_models()" in src
    legs = src.index('_weg2_ph("leg_collects")')
    assert src.index("self.offload_tags.remove(tag)") < legs
    assert legs < src.index("_scratch = self._weg2_zero_local_scratch(_early)")
    assert "_scratch += self._weg2_zero_local_scratch(_late)" in src
    g = helper.index("if any(is_weights_family_tag(t)")
    assert "offload_tags" in helper[g:g + 120]
    assert g < helper.index("zero_local_scratch(_m, residue=residue)")
...
def test_a_partial_weights_chunk_defers_the_memset():
    """UNIFY S2 guard, behaviour: a weights tag still paused -> nothing zeroed
    (and no residue taken); family complete -> zeroed and counted."""
    m = torch.nn.Module()
    m.workspace = torch.nn.Parameter(torch.ones(4, dtype=torch.int32), requires_grad=False)
    fake = types.SimpleNamespace(offload_tags={"weights_3"}, _weg2_draft_unpark_ph0=None,
                                 _weg2_scratch_residue=[0, "all"])
    f = WU.SchedulerWeightUpdaterManager._weg2_zero_local_scratch
    assert f(fake, [m]) == []
    assert int(m.workspace.sum()) == 4 and fake._weg2_scratch_residue == [0, "all"]
    fake.offload_tags = {"kv_cache"}
    assert f(fake, [m]) == ["workspace"]
    assert int(m.workspace.sum()) == 0 and fake._weg2_scratch_residue == [4, "all"]
    # an H25 unpark in flight: zeroed, but no host-syncing count
    m.workspace.data.fill_(1)
    fake._weg2_draft_unpark_ph0 = ([], 0)
    assert f(fake, [m]) == ["workspace"]
    assert int(m.workspace.sum()) == 0 and fake._weg2_scratch_residue == [4, "target"]
