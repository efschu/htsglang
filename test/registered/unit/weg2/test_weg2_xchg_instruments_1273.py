...
"""#1273 slice S7: the remap instrument and the W73 residency line.
...
* **W73.** The exchange's VRAM peak is spec section 5's arithmetic over a
...
class XchgInstrumentTest(CustomTestCase):
    ...
    def test_launch_refuses_when_a_wave_peak_exceeds_the_card(self):
        """W73 by name, naming card, direction, wave, peak, total and floor.
        ...
        self.assertIn("W73 Weg2XchgResidencyUnarmable", msg)
    ...
    def test_w73_leaves_cli_as_the_named_line_and_exit_2(self):
        """The refusal must be ENROLLED in the handler, not merely promise it.

        ROUND-2 REVIEW F1.  W73 shipped as a bare ``RuntimeError``, a subclass
        ...
        self.assertTrue(
            ...
            "W73 is not enrolled in launcher.REFUSALS: it will exit 1",
        )
        self.assertTrue(
            issubclass(
        ...
        def boom(argv=None):
            raise xchg_residency.Weg2XchgResidencyUnarmable(
                "W73 Weg2XchgResidencyUnarmable: synthetic, for the handler only"
            )
        ...
        self.assertIn("W73 Weg2XchgResidencyUnarmable", out)
    ...
    def test_sb4_census_reproduces_the_spec_residency_table(self):
        ...
        hand from boot weg2sb4's own census, then the number W73 refuses on is
    ...
    def test_w73_is_the_only_code_this_slice_allocates(self):
        ...
        self.assertEqual(codes, {("W73", "Weg2XchgResidencyUnarmable")})
