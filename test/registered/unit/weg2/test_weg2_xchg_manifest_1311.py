...
**FINDING 1.**  ``W82 Weg2XchgShadowPlanDiverged scope=oncard-peer
...
``hook=source``, independent of W82.  Its root is the ARM, and the obvious
...
# ---------------------------------------------------------------------------
# The gate itself: W82 must go quiet on a converged pair and LOUD on a real one.
# ---------------------------------------------------------------------------
...
def test_the_gate_no_longer_reports_w80_for_a_converged_pair(chunked, region):
    ...
    the only end that could print W82 -- and it must not, once both ends carry
    the agreed digest."""
    (p_agreed, _), (d_agreed, _) = _agree_both(region)
...
def test_a_pair_that_really_diverges_still_raises_w80(chunked, region):
    """THE DANGER DIRECTION.  A fix that made W82 unreachable would be worse
...
def test_the_first_hook_of_a_boot_refuses_by_name_and_never_diverges(
    ...
    published a zero, the gate would report W82 for pure startup order.  The
...
# ---------------------------------------------------------------------------
# W85 -- the overflow refuses, and never truncates.
# ---------------------------------------------------------------------------
...
def test_the_adapter_reconciles_the_manifest_before_it_derives_the_plan():
    ...
    assert "require_agreement=True" in plan_src, (
        ...
        "rank's own inventory -- the rank-local derivation W82 came from")
