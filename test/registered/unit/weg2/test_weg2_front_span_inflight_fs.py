...
THE FIX (switch ``SGLANG_WEG2_FRONT_SPAN_INFLIGHT``, needs ``..._49``, both
default off): the moment a D leg-2 stream delivers its first CONTENT event, D
...
def _off(monkeypatch):
    monkeypatch.delenv("SGLANG_WEG2_FRONT_SPAN_49", raising=False)
    monkeypatch.delenv("SGLANG_WEG2_FRONT_SPAN_INFLIGHT", raising=False)
...
def test_switches_default_off(monkeypatch):
    _off(monkeypatch)
    assert F.front_span_49() is False
    assert F.front_span_inflight() is False
    # inflight alone does nothing: its credit is #49's held credit
    monkeypatch.setenv("SGLANG_WEG2_FRONT_SPAN_INFLIGHT", "1")
    assert F.front_span_inflight() is False


def test_off_keeps_tools_last(monkeypatch):
    _off(monkeypatch)
    body = {"system": "S", "tools": [{"name": "Read"}],
            "messages": [{"role": "user", "content": "hi"}]}
    t = request_text(body)
    assert t.index("system:") < t.index("user:") < t.index("tools:")
    _on(monkeypatch, inflight=False)
    t = request_text(body)
    assert t.index("tools:") < t.index("system:") < t.index("user:")
    # explicit argument wins over the switch
    assert request_text(body, tools_first=False).endswith(
        "tools:" + json.dumps([{"name": "Read"}], sort_keys=True) + "\n")


def _old_price(text, entries):
    """The unified tree's pricing before the port, verbatim in arithmetic."""
    est = int(len(text) / CHARS_PER_TOKEN) + 1
    best, known = 0, False
    for etext, etok in entries:
        cp = F.common_prefix_len(etext, text)
        if cp <= 0:
            continue
        known = True
        best = max(best, int(etok * (cp / max(1, len(etext)))))
    return max(0, est - best), est, known


def test_off_prices_exactly_as_before_the_port(monkeypatch):
    """With the switch off, the call sites record (text, ct) only and pass no
    epoch; the new SpanLRU then reproduces the old arithmetic exactly."""
    _off(monkeypatch)
    base = "tools:x\n" + "a" * 30000
    entries = [(base, 9000), (base + "b" * 3000, 9500), ("zzz", 5)]
    spans = SpanLRU()
    for t, ct in entries:
        spans.record_presence(t, ct)
    for probe in (base + "b" * 3000 + "c" * 900, base[:20000], "unrelated", base + "q"):
        assert price_remainder(probe, spans) == _old_price(probe, entries), probe


def test_off_call_sites_pass_neither_pt_nor_epoch():
    src = inspect.getsource(Front.leg2)
    assert src.count("prompt_tokens=(pt if _fs49 else 0)") == 2
    assert src.count("held_epoch=(self.epoch if _fs49 and") == 2
    route = inspect.getsource(Front.handle_generate)
    assert "front_span_49() and self.awake == \"D\"" in route
...
def test_leg2_without_the_switch_learns_only_at_the_finish(monkeypatch, caplog, inflight):
    if inflight:
        # #49 off: the inflight switch alone must not write a held credit
        _off(monkeypatch)
        monkeypatch.setenv("SGLANG_WEG2_FRONT_SPAN_INFLIGHT", "1")
    else:
        _on(monkeypatch, inflight=False)
    ...
    # the instrument line does not depend on the switches
