...
def _form(tmp_path, form="DFLASH", produce="off", draft_kv_on_p="on"):
    L.apply_spec_form(SimpleNamespace(
        spec_form=form, dflash_draft_path=str(tmp_path), dflash_block=8, dflash_window=2048,
        dflash_produce_on_p=produce, draft_kv_on_p=draft_kv_on_p))
...
def test_the_producer_is_read_off_ps_form(tmp_path):
    cases = [
        (("DFLASH", "off", "on"), False),   # the 2026-09-24 standard form
        (("DFLASH", "on", "on"), True),     # the old producer form (A/B)
        (("DFLASH", "on", "off"), False),   # P carries no producer flags at all
        (("NEXTN", "off", "on"), True),     # the NEXTN producer always computes
        (("NEXTN", "off", "off"), False),   # the serving-base form
    ]
    for (form, produce, dkv), want in cases:
        _form(tmp_path, form, produce, dkv)
        assert L.p_group_has_draft_producer() is want, (form, produce, dkv)
...
@pytest.mark.parametrize("form,produce", [("DFLASH", "on"), ("NEXTN", "off")])
def test_off_against_a_computing_producer_refuses_by_name(tmp_path, monkeypatch, form, produce):
    # the DFlash producer's first publish would raise (no registered draft
    # pool); the NEXTN one would compute for nothing
    _form(tmp_path, form=form, produce=produce)
    monkeypatch.setenv(ENV, "off")
    with pytest.raises(L.Weg2LaunchRefused, match="W152 Weg2HicacheDraftTierConflict"):
        L.resolve_hicache_draft_tier()


def test_spec_form_env_is_byte_identical_in_both_forms(tmp_path):
    _form(tmp_path, produce="off")
