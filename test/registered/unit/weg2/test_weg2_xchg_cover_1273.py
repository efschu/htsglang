...
"""#1273 slice S2 -- coverage arming (W69) and the draft tag.
...
1. **Coverage.**  The exchange fills the destination's weight pages from the
   ...
   refuses by name (**W69 Weg2XchgCoverageRefused**) rather than ship a tag it
...
class RegionAwareTagTest(_ChunkedCase):
    ...
    def test_coverage_under_a_draft_region_has_no_family_rows(self):
        ...
        W69 can fire over a population that is out of family by construction --
...
class CoverageTest(_ChunkedCase):
    def test_uncovered_tensor_refuses(self):
        ...
        with no source: W69, by name, with the module path in the message."""
        model = _Model()
...
class RollForwardTagTest(unittest.TestCase):
    """Refuter F6: W75's roll-forward opens ONE region for BOTH shards."""
...
class WCodeTest(unittest.TestCase):
    def test_w69_is_the_coverage_refusal_and_says_so_once(self):
        self.assertIn("W69", wms.Weg2XchgCoverageRefused.__doc__ or "")
        self.assertEqual(wx.COVERAGE_REFUSAL_MARKER, "W69 Weg2XchgCoverageRefused")

    def test_w78_is_the_runner_shape_refusal_and_says_so_once(self):
        self.assertIn("W78", wx.Weg2XchgRunnerShapeUnknown.__doc__ or "")
        self.assertEqual(
            wx.RUNNER_SHAPE_REFUSAL_MARKER, "W78 Weg2XchgRunnerShapeUnknown"
        )
