...
buffer line per stage, the draft-group line, the OOM text).
...
class TestVerdicts:
    ...
    def test_the_card_ceiling_at_16384_is_the_edge(self):
        fr, kv, draft, chunk = _form_of("fnFL2x145")
        fits = _solve((0.26, 0.45, 0.733887), kv, draft, chunk)
        assert fits[0].ceiling_max_rows == 212
        assert fits[0].ceiling_fraction == 0.351
        # one row above the edge dies, the edge itself passes
        edge = _solve((0.351, 0.45, 0.733887), kv, draft, chunk)
        assert edge[0].buffer_rows == 212 and not edge[0].refused
        over = _solve((0.353, 0.45, 0.733887), kv, draft, chunk)
        assert over[0].buffer_rows == 213 and over[0].refused
