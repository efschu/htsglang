...
def test_short_piece_is_refused_by_the_consumer(region, sems, ops):
    """The producer posts ``bytes_filled = plan - 1``; the consumer raises W72
    and issues NO copy.
    ...
    it.  A W72 raised after the copy is a log line, not a guard.
    """
    pair = xr.pair_id(0, 1)
    ...
    assert ops.issued == before, "W72 must precede the first copy, not follow it"
...
def test_consumer_never_reads_an_unposted_slot(region, sems, ops):
    ...
    assert "W71 Weg2XchgGateTimeout" in str(excinfo.value)
...
def test_oncard_lane_falls_back_by_name(tmp_path):
    """A failing ``cudaIpcOpenMemHandle`` arms **W74** at LAUNCH and the lane
    ...
    # runtime use the W74 CLASS has.  Degrading here would run a boot whose
...
def test_the_degraded_lane_really_moves_the_bytes(tmp_path, region):
    ...
    try:
        ...
        # and it scales exactly as the W74 line's `host_add_mib` claims.
...
def test_batches_are_the_same_whether_the_side_coalesced_or_not():
    ...
    If this were false, EVERY boot would fire W72 and the refusal would be
...
def test_a_run_that_cannot_fit_a_slot_is_refused_before_any_copy():
    ...
    assert "W70 Weg2XchgPlanDisagree" in str(excinfo.value)
...
def test_a_batch_never_exceeds_the_slot_it_will_be_written_into():
    ...
    that over-packed would turn every flip into a W70 -- but only after the
...
def test_oncard_consumer_refuses_a_short_hop(region, tmp_path):
    """W72 on the diagonal too -- one refusal, both lanes.
...
def test_a_second_batch_does_not_overwrite_the_first_batchs_row(region):
    ...
    sequence and then compares byte counts -- raised W72 against a producer
...
def test_an_armed_row_is_the_handle_signal_and_not_a_batch(region):
    ...
    W72 against a producer that had done nothing wrong.
    """
    tp.write_oncard_row(region, tp.DIR_ONCARD_PROD_OFF, 0, seq=-1, nbytes=0,
                        slot_bytes=SLOT, wave=WAVE,
                        state=tp.ONCARD_STATE_ARMED)
...
def test_a_previous_waves_oncard_row_is_not_this_waves_signal(region):
    ...
    * FILL, reachable whenever a wave's on-card share is two batches or fewer:
      the stale row matches EXACTLY and either raises W72 against a healthy
...
def test_the_sequence_half_of_the_short_piece_check_fires_on_its_own(
        region, sems, ops):
    """W72's ``rec.seq != batch.seq`` half, alone.
    ...
    assert "seq_ok=no" in message, (
        "a W72 that printed only byte counts would read as a contradiction of "
        "itself here")
    assert ops.issued == before
    assert read(ops, dst, len(payload)) != payload
...
def test_a_signal_during_a_slot_wait_is_not_a_timeout(boot):
    ...
    ``W71 ... no producer posted this slot within the fence budget`` naming a
...
def test_every_thread_that_failed_is_reported_not_only_the_first(
    ...
    assert "W71 Weg2XchgGateTimeout" in extra[0]
...
def test_every_w_code_this_slice_raises_is_free_and_named_once():
    """W72 and W74 are what spec section 7 assigns to S4, and the branch census
    at 2ee844f7b8 leaves exactly those two free below W78 (W69/52/53/55/58/60
    are held by S1/S2/S3/S7; W75 is S6's and W77 is S5's).
    ...
    """
    assert tp.SHORT_PIECE_MARKER == "W72 Weg2XchgShortPiece"
    assert tp.ONCARD_UNAVAILABLE_MARKER == "W74 Weg2XchgOnCardUnavailable"
    ...
    assert found == {
        ...
        # W80 is free in the branch census -- W78 was the highest assigned and
        # W79 is the shadow's.
        ...
        # assigned codes with a maximum of W82, so the next free code is W83.
        # The gaps below that maximum (W5, W6, W13-15, W18, W19, W23, W24,
        # W27, W39, W75) are deliberately NOT reused: a retired number still
        ...
        # S6 fix E (#1273): the on-card slot CEILING flag's refusal.  W83 was
        # the census maximum when the comment above was written, so W84 is the
...
def test_a_deposit_with_fewer_slots_than_batches_is_refused_before_a_copy(
        tmp_path, region):
    """S6 W83: the danger direction, refused where it cannot be compensated.
    ...
    try:
        ...
        bounce.close()
        assert "W83 Weg2XchgDepositUnfundable" in str(caught.value)
...
def test_the_slot_count_is_derived_from_the_batches_and_prints_its_provenance():
    ...
    count IS the batch count, and the plan is its one producer, so the W70
...
def test_the_two_sides_ring_depths_are_compared_and_not_assumed(
    ...
    budget in a W71 whose denominator names an ABSENT peer -- never a ring
    ...
    try:
        ...
        assert "W70 Weg2XchgPlanDisagree" in message
        assert "slots=4" in message and "derived 2" in message
        assert "different depths" in message

        # THE CAN-FAIL CONTROL: agreeing depths do NOT raise W70.  This rank
