...
def test_a_pipeline_depth_the_rows_cannot_address_is_refused_before_any_copy(
        region, tmp_path):
    """W70 by name, and the refusal lands before a thread exists."""
...
def test_a_diagonal_too_large_for_the_ceiling_is_priced_red_not_rounded_green():
    ...
    W73 has to refuse later -- so it clamps and says ``fits=no``.
    """
    huge = 200 * (1 << 30)
...
def test_the_gate_expiry_switches_the_shadow_off_and_never_raises(region):
    ...
    ``WEG2_GROUP_FENCE_BUDGET_S`` and then raise W71 -- correct there, fatal
...
def test_shadow_mismatch_names_the_parameter():
    ...
    for token in ("W77 Weg2XchgShadowMismatch", "class=in_proj_qkvz",
...
def test_a_transport_failure_is_logged_and_never_raised(region, tmp_path, boot):
    ...
    W70/W71/W72 stop a flip on the authoritative path.  Here the same events
...
def test_a_stale_full_count_refuses_the_leg_by_name(boot):
    ...
    A flip abandoned after gate 1 rolls forward by design (W75) and leaves any
...
def test_the_slot_checksum_asks_representability_before_it_says_mismatch():
    ...
    reaches the same W77 marker -- logged on a bare inequality.  A batch whose
...
def test_the_diagonal_slot_is_this_cards_lane_and_not_the_sum():
    ...
    # descriptors, which is what keeps the W70 slot_bytes check a cross-check.
...
def test_the_plan_seam_has_exactly_one_producer_and_it_is_the_derivation():
    ...
    which is precisely why it was written as a DENOMINATOR test: the W81
...
def test_a_rank_that_cannot_join_says_so_by_name_and_does_not_go_quiet(
        no_active_leg):
    """DANGER 4.  Every local skip is W81 with its reason, never a bare return.
...
def test_an_explicit_caller_gets_the_skip_raised_and_the_leg_never_does(
        no_active_leg):
    """The W74/W79 two-arm shape, third instance: automatic degrades, explicit raises."""
...
def test_a_priced_hop_over_the_bound_refuses_the_shadow_by_name_and_not_the_flip(
    ...
    """W79 ``scope=hop``: the leg is refused BEFORE a wall is spent.
...
def test_a_stale_semaphore_stops_the_shadow_and_never_the_flip(region, boot,
    ...
    so the hook catches it, prints it, and reports ``reason=w80-stale`` with
    ...
    try:
        ...
        assert result.reason == "w80-stale"
...
def test_the_destination_hook_cannot_swallow_a_mismatch(region, tmp_path, boot,
    ...
    (``verdict=MISMATCH``), the W77 marker must be on the log, and nothing may
...
# --- W81 is a new, free code ----------------------------------------------

def test_w81_is_the_next_free_code_and_names_one_exception():
    assert sh.RANK_LOCAL_SKIP_MARKER == "W81 Weg2XchgShadowRankLocalSkip"
    assert sh.Weg2XchgShadowRankLocalSkip.__name__ in sh.RANK_LOCAL_SKIP_MARKER
    # The TIME term reuses W79 rather than taking a code of its own: one code
...
def test_a_refusal_after_the_attach_publishes_its_no_so_no_peer_waits(
    ...
    flip legs.  W81's own docstring names the hazard; the code only logged it.
    """
    xr.create_semaphores(boot)
    sems = tp.SemSet(boot)
    try:
...
def test_a_refusal_before_the_attach_has_nowhere_to_publish_and_admits_it(
    ...
    with no row to write into, and W81 names them instead of the code
...
def test_a_plan_for_a_card_this_rank_does_not_run_on_is_refused(chunked):
    ...
    group layout has, ``_blocks_of`` would raise W70 deep inside ``build_plan``
...
def test_a_stale_flip_order_map_is_refused_by_name(chunked):
    ...
    not have, and ``build_plan``'s own W76 would then refuse for a reason
...
def test_the_class_rotation_moves_the_card_digest_and_not_the_group_one(chunked):
    ...
    surfaced as W82 ``scope=group`` under a cause sentence naming a stale chunk
...
def test_a_divergent_plan_is_a_named_refusal_and_never_a_vote(region):
    """DANGER 4 at the rendezvous: W82, ``scope=group``, no vote taken.
...
def test_the_co_located_pair_is_checked_on_its_card_geometry(region):
    """W82 ``scope=oncard-peer``: the two ends of ONE card's lane.
    ...
    W72 byte-count disagreement inside the on-card consumer, after the source
...
def test_the_compare_names_the_class_the_stripes_and_the_bytes(
    ...
    W77 names a class only when something went WRONG, so a boot whose shadow
...
def test_the_blocked_wall_reaches_the_shadow_line_from_a_failed_leg(tmp_path,
    ...
    result = tp.LegResult()
    result.oncard = stats
    exc = tp.Weg2XchgGateTimeout("W71 Weg2XchgGateTimeout oncard -- synthetic")
...
def test_w82_is_the_next_free_code_and_names_one_exception():
    """W82, and it is not folded into the class-subset disagreement.

    W81 was the highest assigned code on this branch; W82 is the first free
    one above it, per ``test_weg2_wcode_uniqueness_1263``'s census rule.
    """
    assert sh.PLAN_DIVERGED_MARKER == "W82 Weg2XchgShadowPlanDiverged"
...
def test_an_unfunded_deposit_is_refused_by_name_and_moves_no_bytes(
    ...
    """S6 W83: pinned host bytes nobody charged for are not a risk to accept.
    ...
    assert result.ran is False
    refusal = [ln for ln in lines if ln.startswith("W83 Weg2XchgDepositUnfundable")]
    assert len(refusal) == 1, lines
...
def test_a_deposit_with_more_batches_than_slots_is_refused_by_name(
    ...
    """S6 W83: the second refusal arm, at a slot the row area cannot cover.
...
# zero COMPARE, zero W79/W83.  The ledger half of both arms was exact, so the
# arm itself was real.
#
...
#
#   W70 Weg2XchgPlanDisagree begin_flip epoch='1788964408.-1': flip index -1
...
# (14:33:55Z), 73 s BEFORE the first flip began (14:35:08Z) -- and the W81 that
...
def test_a_leg_with_no_flip_epoch_refuses_by_name_before_the_sentinel_travels(
    ...
    """RED ON `57ef547466`: the -1 travels and comes back as W70.
    ...
    assert "Weg2XchgPlanDisagree" not in caplog.text, (
        ...
        "shadowD W70, which names a plan disagreement on a leg that is simply "
        "not a flip"
    )
...
def test_every_identity_the_hook_reads_has_a_named_refusal(
    ...
    W79 line as though it were a real one) and `free_mib=0` (an UNREADABLE NVML
...
# subset rotates A_log/down_proj, W70 0, W79 0, 19 AFFORDABLE / 5 REFUSED,
...
#
# WHAT THE GATE FOUND IS REAL, AND THE GATE IS RIGHT.  W82 scope=oncard-peer
# compares the CO-LOCATED pair's `card_digest`:
#
#     W82 Weg2XchgShadowPlanDiverged scope=oncard-peer leg=0 field=card_digest
...
def test_a_mismatched_piece_set_still_diverges_and_names_a_piece():
    """The W72 hazard the gate exists for survives the rebuild.
