"""Without SGLANG_HTCCL* nothing of the BAR1 strand happens.
...
1. **Import.** A fresh interpreter that builds the distributed stack, or
   the MoE dispatcher package, must not pull in ``htccl_bar1*``. Those are
...
4. **Flags.** Every new environment variable defaults to the previous
   behaviour, and their names all carry the ``SGLANG_HTCCL`` prefix, so
   "unset every SGLANG_HTCCL*" really is the whole off switch.
...
def _clean_env():
    """The environment without any SGLANG_HTCCL* / bar1 knob."""
    env = {
        k: v
        for k, v in os.environ.items()
        if not k.startswith("SGLANG_HTCCL") and "BAR1" not in k
    }
...
class TestNothingIsImported(CustomTestCase):
    ...
    def test_distributed_import_does_not_pull_the_bar1_modules(self):
        code = (
            ...
            "import sglang.srt.distributed.device_communicators.htccl\n"
            "import sys\n"
    ...
    def test_moe_import_does_not_pull_the_transport(self):
        ...
        What must NOT come with it is the transport side: ``htccl_bar1`` and
        ...
        code = (
            ...
            "import sys\n"
            "print(' '.join(m for m in sys.modules if 'htccl_bar1' in m))\n"
        )
        self.assertEqual(self._imports(code, _clean_env()), set())
    ...
    def test_the_modules_are_importable_at_all(self):
        ...
        code = (
            ...
            "for m in ('sglang.srt.distributed.device_communicators.htccl_bar1',"
            "'sglang.srt.layers.moe.token_dispatcher.bar1ep'):\n"
            "    importlib.import_module(m)\n"
            "print(' '.join(m for m in sys.modules if 'bar1' in m))\n"
        )
        got = self._imports(code, _clean_env())
        self.assertTrue(any("htccl_bar1" in m for m in got))
    ...
    def test_every_op_falls_to_the_gloo_plane(self):
        ...
        for op in (
            ...
            for nbytes in (0, 4096, 10600448):
                self.assertIsNone(HTCCLCommunicator._select(c, op, nbytes))


class TestFlagNamesAndDefaults(CustomTestCase):
    def test_every_new_bar1_knob_is_under_the_htccl_prefix(self):
        """ "Unset every SGLANG_HTCCL*" has to be the complete off switch.
        ...
        """
        offenders = []
        for path in sorted(_COMM.glob("htccl_bar1*.py")) + [
            ...
            for name in re.findall(
                ...
                if not name.startswith("SGLANG_HTCCL"):
    ...
    def test_all_gather_defaults_on_inside_bar1_and_is_switchable(self):
        ...
        self.assertIn("all_gather", HTCCLBar1Transport.HTCCL_OPS)
        src = (_COMM / "htccl_bar1.py").read_text(encoding="utf-8")
        self.assertIn('os.environ.get("SGLANG_HTCCL_BAR1_AG", "1")', src)
