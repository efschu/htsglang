...
Context. Before this, ``HTCCLBar1Transport.HTCCL_OPS`` covered only
...
class TestOpRegistration(CustomTestCase):
    ...
    def test_ops_and_methods_agree(self):
        ...
        for op in HTCCLBar1Transport.HTCCL_OPS:
            name = f"htccl_{op}"
            if op == "all_to_all":
                # Two spellings, one path -- documented at HTCCL_OPS.
...
class TestAgPlanArithmetic(CustomTestCase):
    ...
    def test_the_handover_payload(self):
        ...
        8 384 512 B is what the window-geometry helper in htccl_bar1.py
...
class TestLoudBarStillGuardsTheRest(CustomTestCase):
    """The bar in htccl._select, on the ops all_gather did NOT cover."""
    ...
    def test_uncovered_op_under_capture_raises_and_names_the_coverage(self):
        ...
        # The message reads HTCCL_OPS, so it cannot go stale when an op is
