...
"""barlink UCX transport: collective correctness + registry wiring (task #117).
...
class _Comm:
    ...
    Mirrors BarlinkCommunicator._get_out_buf exactly. The real communicator
...
def _worker(rank, world, store, q):
    ...
    try:
        ...
        comm = _Comm()
        t = BarlinkUcxTransport(cpu_group=dist.group.WORLD, device=torch.device("cpu"))
        ...
        if world > 2:
            ...
            chk("all_gather/ring/3d-dim2",
                t.barlink_all_gather(comm, three[rank].clone(), 2),
                torch.cat(three, dim=2))
            ring_a = t.barlink_all_gather(comm, parts[rank].clone(), 0)
            ring_snap = ring_a.clone()
            ring_b = t.barlink_all_gather(comm, (parts[rank] * 3).clone(), 0)
        ...
        for label, count in (
            ...
            chk(f"all_gather/pipeline/{label}",
                t.barlink_all_gather(comm, gramp[rank].clone(), 0),
        ...
        for dim in (0, 1, -1):
            chk(f"all_gather/pipeline/2d-dim={dim}",
                t.barlink_all_gather(comm, gramp2[rank].clone(), dim),
        ...
        gramp = [torch.arange(9000, dtype=torch.float32) * (r + 3)
                 for r in range(world)]
        g_piped = t.barlink_all_gather(comm, gramp[rank].clone(), 0)
        was = t.pipeline  # restore, never assign True back
        t.pipeline = False
        g_plain = t.barlink_all_gather(comm, gramp[rank].clone(), 0)
        t.pipeline = was
        ...
        for dim in (0, 1):
            g_fast = t.barlink_all_gather(comm, small[rank].clone(), dim)
            was = t.pipeline
            t.pipeline = False
            g_gen = t.barlink_all_gather(comm, small[rank].clone(), dim)
            t.pipeline = was
...
class TestUcxRegistryWiring(CustomTestCase):
    ...
    def test_handles_is_size_independent(self):
        ...
        fake = BarlinkUcxTransport.__new__(BarlinkUcxTransport)
        for op in ("all_reduce", "all_gather", "broadcast", "reduce_scatter"):
            for nbytes in (0, 1, 1 << 10, 1 << 30):
                self.assertTrue(BarlinkUcxTransport.handles(fake, op, nbytes))
        for nbytes in (0, 1 << 30):
            self.assertFalse(BarlinkUcxTransport.handles(fake, "nonsense", nbytes))
    ...
    def test_dispatch_selects_ucx(self):
        ...
        comm.transport = fake
        self.assertIs(BarlinkCommunicator._select(comm, "all_reduce", 1 << 20), fake)
        self.assertIsNone(BarlinkCommunicator._select(comm, "nonsense", 8))
...
class TestUcxVersionParity(CustomTestCase):
    ...
    def _check(self, gathered):
        ...
        fake = BarlinkUcxTransport.__new__(BarlinkUcxTransport)
        return BarlinkUcxTransport._check_version_parity(fake, gathered)
