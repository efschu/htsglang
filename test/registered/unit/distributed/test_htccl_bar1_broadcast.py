...
Context. Before this, ``HTCCLBar1Transport.HTCCL_OPS`` covered all_reduce,
...
a pure function, ``htccl_broadcast`` is driven against a stub that records the
...
def _fahre(t, tensor, src):
    """Run ``htccl_broadcast`` on a stub and collect the tables it builds.
    ...
    with mock.patch.object(type(t), "htccl_all_to_all_single", _a2a):
        ergebnis = t.htccl_broadcast(None, tensor, src)
...
class TestOpRegistration(CustomTestCase):
    ...
    def test_broadcast_is_covered(self):
        self.assertIn("broadcast", HTCCLBar1Transport.HTCCL_OPS)
    ...
    def test_ops_and_methods_agree(self):
        for op in HTCCLBar1Transport.HTCCL_OPS:
            name = f"htccl_{op}"
            if op == "all_to_all":
                name = "htccl_all_to_all_single"
            self.assertTrue(
                hasattr(HTCCLBar1Transport, name),
                msg=f"HTCCL_OPS claims {op!r} but there is no {name}()",
            )
...
class TestTheTableItBuilds(CustomTestCase):
    ...
    ``htccl_broadcast`` is driven against a stub that records what it would
...
class TestLoudBar(CustomTestCase):
    """The bar in htccl._select, before and after this change."""
    ...
    def test_reduce_scatter_still_raises_and_names_the_new_coverage(self):
        ...
        text = str(ctx.exception)
        self.assertIn("reduce_scatter", text)
        # Derived from HTCCL_OPS, never a literal list in the message -- so it
