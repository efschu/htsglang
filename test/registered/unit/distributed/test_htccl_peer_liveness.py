...
* the gloo ``cpu_group`` every HTCCL handshake runs on is built with a
...
from sglang.srt.distributed.device_communicators import htccl_liveness as live
...
class _EnvGuard:
    ...
    ``SGLANG_HTCCL_PEER_LIVENESS=0`` -- that path is an unbounded loop, so
...
class TestBoundedPollDeadline(LivenessTestBase):
    def test_deadline_fires_and_names_the_wait(self):
        with _EnvGuard(SGLANG_HTCCL_PEER_TIMEOUT_S=0.3, SGLANG_HTCCL_PEER_PROBE_S=0.05):
    ...
    def test_deadline_error_names_the_knob_to_change(self):
        with _EnvGuard(SGLANG_HTCCL_PEER_TIMEOUT_S=0.2):
            with self.assertRaises(live.CollectiveTimeoutError) as ctx:
                live.bounded_poll(lambda: False, "never-ready")
    ...
    def test_a_wait_that_resolves_late_still_succeeds(self):
        ...
        with _EnvGuard(
            SGLANG_HTCCL_PEER_TIMEOUT_S=10.0, SGLANG_HTCCL_PEER_PROBE_S=0.05
        ):
    ...
    def test_peer_census_is_in_the_timeout_message(self):
        ...
        table = self.table_with([self.me(0), peer], self_rank=0)
        with _EnvGuard(SGLANG_HTCCL_PEER_TIMEOUT_S=0.2, SGLANG_HTCCL_PEER_PROBE_S=0.05):
            with self.assertRaises(live.CollectiveTimeoutError) as ctx:
...
class TestBoundedPollPeerLoss(LivenessTestBase):
    ...
    def test_dead_peer_beats_the_deadline(self):
        ...
        with _EnvGuard(
            SGLANG_HTCCL_PEER_TIMEOUT_S=600.0, SGLANG_HTCCL_PEER_PROBE_S=0.05
        ):
            started = time.monotonic()
    ...
    def test_peer_loss_trips_registered_abort_windows(self):
        ...
        table = self.table_with([self.me(0), peer], self_rank=0)
        with _EnvGuard(
            SGLANG_HTCCL_PEER_TIMEOUT_S=600.0, SGLANG_HTCCL_PEER_PROBE_S=0.0
        ):
...
class TestSuccessPathCost(LivenessTestBase):
    ...
    def test_the_probe_is_rate_limited_not_per_iteration(self):
        """Cost ceiling: at most one census per SGLANG_HTCCL_PEER_PROBE_S.
        ...
        with mock.patch.object(live, "pid_alive", counting):
            with _EnvGuard(
                SGLANG_HTCCL_PEER_TIMEOUT_S=0.6, SGLANG_HTCCL_PEER_PROBE_S=0.2
            ):
...
class TestKillSwitch(LivenessTestBase):
    def test_disabled_restores_the_unbounded_loop(self):
        ...
        with _EnvGuard(SGLANG_HTCCL_PEER_LIVENESS=0, SGLANG_HTCCL_PEER_TIMEOUT_S=0.001):
    ...
    def test_disabled_never_raises_on_a_dead_peer(self):
        peer = live.PeerIdentity(rank=1, host=live._hostname(), pid=4194305, boot="")
        table = self.table_with([self.me(0), peer], self_rank=0)
        with _EnvGuard(SGLANG_HTCCL_PEER_LIVENESS=0):
    ...
    def test_zero_timeout_also_means_unbounded(self):
        ...
        with _EnvGuard(SGLANG_HTCCL_PEER_TIMEOUT_S=0):
    ...
    def test_watchdog_is_separately_switchable(self):
        with _EnvGuard(SGLANG_HTCCL_PEER_LIVENESS=1, SGLANG_HTCCL_PEER_WATCHDOG=0):
...
class TestColdBuildWindow(LivenessTestBase):
    ...
    def test_deadline_is_scaled_inside_the_window(self):
        ...
        with _EnvGuard(
            SGLANG_HTCCL_PEER_TIMEOUT_S=100.0, SGLANG_JIT_COLD_BUILD_TIMEOUT_MULT=7
        ):
    ...
    def test_a_dead_peer_is_still_caught_inside_the_window(self):
        ...
        table = self.table_with([self.me(0), peer], self_rank=0)
        with _EnvGuard(
            SGLANG_HTCCL_PEER_TIMEOUT_S=600.0,
            SGLANG_JIT_COLD_BUILD_TIMEOUT_MULT=40,
            SGLANG_HTCCL_PEER_PROBE_S=0.0,
        ):
            with jit_cold_build.cold_build_window("test"):
                started = time.monotonic()
                with self.assertRaises(live.PeerLostError):
...
class TestConfigSurface(LivenessTestBase):
    def test_every_knob_is_registered_in_environ(self):
        ...
        for name in (
            ...
            "SGLANG_HTCCL_PEER_PROBE_S",
            "SGLANG_HTCCL_PEER_WATCHDOG",
        ):
            self.assertTrue(hasattr(envs, name), f"{name} missing from environ.py")

    def test_env_names_match_the_module_constants(self):
        self.assertEqual(live.ENV_ENABLE, "SGLANG_HTCCL_PEER_LIVENESS")
        self.assertEqual(live.ENV_TIMEOUT_S, "SGLANG_HTCCL_PEER_TIMEOUT_S")
        self.assertEqual(live.ENV_PROBE_S, "SGLANG_HTCCL_PEER_PROBE_S")
        self.assertEqual(live.ENV_WATCHDOG, "SGLANG_HTCCL_PEER_WATCHDOG")

    def test_defaults_are_the_documented_ones(self):
        with _EnvGuard(
            SGLANG_HTCCL_PEER_LIVENESS=None,
            SGLANG_HTCCL_PEER_TIMEOUT_S=None,
            SGLANG_HTCCL_PEER_PROBE_S=None,
            SGLANG_HTCCL_PEER_WATCHDOG=None,
        ):
            self.assertTrue(live.liveness_enabled())
    ...
    def test_a_garbage_value_falls_back_to_the_default(self):
        with _EnvGuard(SGLANG_HTCCL_PEER_TIMEOUT_S="not-a-number"):
            self.assertEqual(live.wait_timeout_s(), 120.0)
