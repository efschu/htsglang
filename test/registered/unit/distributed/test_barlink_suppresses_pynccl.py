"""When barlink is active, the PyNccl communicator must not be CONSTRUCTED.
...
cross-vendor host: both ranks logged "barlink enabled ... (transport=gloo)" and
...
Three properties are pinned here, and the second matters most:
 ...
 1. barlink active  -> pynccl is NOT constructed.
 ...
 3. barlink inactive -> byte-identical to before. pynccl is the faster path on
    same-vendor rigs; the rule is "barlink active -> no pynccl", never
    "no pynccl".
...
class TestBarlinkSuppressesPynccl(CustomTestCase):
    def test_barlink_active_means_no_pynccl(self):
        """Property 1 -- the crash this prevents."""
        for world in (2, 3, 4, 8):
            self.assertFalse(
                _should_build_pynccl(True, world, barlink_active=True),
                f"world={world}: pynccl must not be constructed under barlink",
            )
    ...
    def test_flag_off_is_unchanged(self):
        ...
        for world in (2, 3, 4, 8):
            self.assertTrue(
                _should_build_pynccl(True, world, barlink_active=False),
                f"world={world}: pynccl must still be built without barlink",
            )
        # And the pre-existing conditions still gate it.
        self.assertFalse(_should_build_pynccl(False, 4, barlink_active=False))
        self.assertFalse(_should_build_pynccl(True, 1, barlink_active=False))
        ...
        for use_pynccl in (True, False):
            for world in (1, 2, 4):
                for barlink in (True, False):
                    verdicts = {
                        _should_build_pynccl(use_pynccl, world, barlink)
                    ...
                    self.assertEqual(
                        ...
                        f"use_pynccl={use_pynccl} world={world} barlink={barlink}",
                    )

    def test_old_semantics_would_have_crashed(self):
        """Pin the BUG, so a revert cannot pass silently.

        The old condition omitted the barlink term. Assert that it would have
        ...
        self.assertFalse(
            _should_build_pynccl(True, 2, barlink_active=True),
    ...
    def test_matches_parallel_state_source(self):
        ...
        self.assertIn(
            "should_build_pynccl(use_pynccl, self.world_size, _barlink_active)", src
        )
        # ...and _barlink_active must come from what was actually BUILT, not from
        # re-reading the env, which could diverge from it.
        self.assertIn("_barlink_active = self.barlink_comm is not None", src)
