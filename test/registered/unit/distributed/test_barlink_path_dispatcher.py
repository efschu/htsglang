"""#279 barlink path dispatcher skeleton: registry, latched dispatch, loaders.
...
class TestPlaceholderNeutrality(CustomTestCase):
    ...
    def test_barlink_select_identical_with_and_without_dispatcher(self):
            ...
            for transport in (None, FakeDevice(), FakeShm()):
                for op in ("all_reduce", "all_gather", "reduce_scatter", "broadcast"):
                    for nbytes in (0, 4 * KIB, 64 * MIB, 65 * MIB):
                        ...
                        baseline = BarlinkCommunicator._select(comm, op, nbytes)
                        comm2 = BarlinkCommunicator.__new__(BarlinkCommunicator)
                        comm2.transport = transport
                        comm2._path_dispatcher = dispatcher
                        self.assertIs(
                            BarlinkCommunicator._select(comm2, op, nbytes),
                            baseline,
...
class TestRefineTransportChoice(CustomTestCase):
    """The thin barlink hook, including the measured-decision actuation."""
