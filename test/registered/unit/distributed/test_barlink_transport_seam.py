"""barlink transport seam: pluggable (`shm | device | ucx`), no leaked assumptions.
...
class _FakeDevice:
    """Mirrors BarlinkDeviceTransport's declared capability."""

    BARLINK_OPS = frozenset(OPS)
...
class _FakeShm:
    """Mirrors BarlinkShmTransport's declared capability."""
    ...
    def handles(self, op, nbytes):
        return op in self.BARLINK_OPS and nbytes <= self.slot_bytes


def _select(transport, op, nbytes):
    """Drive BarlinkCommunicator._select without building a real communicator."""
    comm = BarlinkCommunicator.__new__(BarlinkCommunicator)
    comm.transport = transport
    return BarlinkCommunicator._select(comm, op, nbytes)
...
class TestRegistry(CustomTestCase):
    ...
    def test_a_new_transport_becomes_dispatchable_without_touching_call_sites(self):
        ...
        class _FakeUcx:
            BARLINK_OPS = frozenset({"all_reduce", "all_gather"})
...
class TestFallbackPolicy(CustomTestCase):
    ...
    def test_no_fallback_is_exactly_the_capturable_set(self):
        ...
        rule is exactly CAPTURABLE_BARLINK_TRANSPORTS"). A test that has to be
...
class TestRealTransportsDeclareTheExpectedCapability(CustomTestCase):
    ...
    def test_shm_class_declares_all_reduce_only(self):
        ...
        self.assertEqual(BarlinkShmTransport.BARLINK_OPS, frozenset({"all_reduce"}))
        self.assertTrue(hasattr(BarlinkShmTransport, "handles"))
    ...
    def test_device_class_declares_all_three(self):
        ...
        # PyNccl is suppressed under barlink. The device transport declares and
        # implements a capturable one (all_gather + src slice).
        self.assertEqual(
            BarlinkDeviceTransport.BARLINK_OPS,
            frozenset(OPS) | {"broadcast"},
        )
        for name in ("handles", "barlink_all_reduce", "barlink_all_gather",
                     "barlink_reduce_scatter", "barlink_broadcast"):
            self.assertTrue(hasattr(BarlinkDeviceTransport, name), name)
    ...
    def test_device_broadcast_is_built_on_a_byte_exact_primitive(self):
        ...
        fn = ast.parse(
            textwrap.dedent(
                inspect.getsource(BarlinkDeviceTransport.barlink_broadcast)
            )
        ).body[0]
