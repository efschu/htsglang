...
-------------------------------------------------------
``barlink_device._load_ext`` builds the barlink device extension with
...
_NAME = "barlink_device_ext_cuda_86_120"
...
class BarlinkExtCacheSelfHealTest(CustomTestCase):
    def setUp(self):
    ...
    def test_a_complete_entry_of_our_own_is_never_touched(self):
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            warm = _complete(root / "barlink_device_ext_cuda_75", "barlink_device_ext_cuda_75")
            _residue(root / _NAME)

            self._load(root)

            self.assertTrue(
                (warm / "barlink_device_ext_cuda_75.so").is_file(),
                "a warm barlink extension was discarded -- 150 s of nvcc per boot",
            )
    ...
    def test_a_co_located_peers_live_build_is_never_touched(self):
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            peer = _building(root / "barlink_device_ext_cuda_120", os.getpid())
            _residue(root / _NAME)
...
class ScopedSweepTest(CustomTestCase):
    ...
    def test_name_filter_confines_the_sweep(self):
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            mine = _residue(root / "barlink_device_ext_cuda_86")
            theirs = _residue(root / "vendor_ext")

            removed = sweep_cache_root(
                root,
                name_filter=lambda n: n.startswith("barlink_device_ext"),
    ...
    def test_a_pyd_counts_as_a_finished_artifact(self):
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            d = _residue(root / "barlink_device_ext_win")
            (d / "barlink_device_ext_win.pyd").write_bytes(b"MZ")
            self.assertEqual(
    ...
    def test_only_our_own_purge_debris_is_collected(self):
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            mine = root / ".__sglpurge-1234-5678-barlink_device_ext_cuda_86"
            ...
            sweep_cache_root(
                root, name_filter=lambda n: n.startswith("barlink_device_ext")
            )
