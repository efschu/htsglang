"""When HTCCL is active, CustomAllreduce must not be CONSTRUCTED.

Sibling of test_htccl_suppresses_pynccl.py, and the failure mode is the worse
...
class TestHtcclSuppressesCustomAllreduce(CustomTestCase):
    def test_htccl_active_means_no_custom_allreduce(self):
        """Property 1 -- the hang this prevents."""
        for world in (2, 3, 4, 5, 8):
            self.assertFalse(
                _should_build_ca(True, world, htccl_active=True),
                f"world={world}: CustomAllreduce must not be constructed under HTCCL",
            )
    ...
    def test_flag_off_is_unchanged(self):
        ...
        for world in (2, 3, 4, 5, 8):
            self.assertTrue(
                _should_build_ca(True, world, htccl_active=False),
                f"world={world}: CustomAllreduce must still be built without HTCCL",
            )
        self.assertFalse(_should_build_ca(False, 4, htccl_active=False))
        self.assertFalse(_should_build_ca(True, 1, htccl_active=False))
        ...
        for use_ca in (True, False):
            for world in (1, 2, 4, 5):
                for htccl in (True, False):
                    verdicts = {
                        _should_build_ca(use_ca, world, htccl)
                    ...
                    self.assertEqual(
                        ...
                        f"use_ca={use_ca} world={world} htccl={htccl}",
                    )
    ...
    def test_old_semantics_would_have_hung(self):
        ...
        self.assertFalse(
            _should_build_ca(True, 5, htccl_active=True),
    ...
    def test_matches_parallel_state_source(self):
        ...
        self.assertIn("use_custom_allreduce, self.world_size, _htccl_active", src)
