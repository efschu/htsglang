...
class TestTheSeamStagingIsPricedNotDiscovered(CustomTestCase):
    """#665-F1 item 7: the chunk-width term in the seam's memory model.

    The funding solver used to learn what a chunk-size change costs by staging,
    being refused and abandoning -- on a live boot. Moving 512 -> 2048 took the
    pp_to_tp staging want from ~907 MiB to 2481 and made the return leg
    unfundable at 15:14 with nothing having predicted it.
    """

    ANCHORS = ((128, 856), (512, 907), (2048, 2481))

    def _cfg_seam(self, chunk):
        return _cfg(decode_contention=1.0, flip_cost_s=5.918,
                    prefill_chunk_tokens=chunk)

    def test_it_reproduces_every_measured_anchor(self):
        from sglang.srt.managers.phase_policy import seam_staging_want_mib

        for chunk, measured in self.ANCHORS:
            got = seam_staging_want_mib(self._cfg_seam(chunk))
            with self.subTest(chunk=chunk):
                self.assertLess(
                    abs(got - measured), 25,
                    f"chunk {chunk}: predicted {got:.0f} vs measured {measured}",
                )

    def test_a_linear_model_would_not_have_done(self):
        """Why c^2 and not c: the shape, not just the residual.

        A straight line through the outer anchors over-prices 512 and
        under-prices 2048 -- it gets the direction of the error wrong at both
        ends, which is exactly how a chunk change slips through a check.
        """
        lo, hi = self.ANCHORS[0], self.ANCHORS[-1]
        slope = (hi[1] - lo[1]) / (hi[0] - lo[0])
        linear_at_512 = lo[1] + slope * (512 - lo[0])
        self.assertGreater(abs(linear_at_512 - 907), 100)

    def test_the_headroom_includes_the_floor_it_may_not_spend(self):
        from sglang.srt.managers.phase_policy import (
            seam_headroom_needed_mib,
            seam_staging_want_mib,
        )

        cfg = self._cfg_seam(2048)
        want = seam_staging_want_mib(cfg)
        self.assertAlmostEqual(
            seam_headroom_needed_mib(cfg), want + cfg.corridor_floor_mib, places=3
        )
        # The 15:14 incident in one assertion: 2048 needs more free than the
        # instance had (2510), which is why the return leg could not fund.
        self.assertGreater(seam_headroom_needed_mib(cfg), 2510)
        self.assertLess(seam_headroom_needed_mib(self._cfg_seam(512)), 2510)

    def test_an_unknown_chunk_width_is_not_priced_as_free(self):
        from sglang.srt.managers.phase_policy import seam_staging_want_mib

        self.assertEqual(seam_staging_want_mib(self._cfg_seam(0)), 0.0)

    def test_it_is_monotonic_in_the_chunk_width(self):
        from sglang.srt.managers.phase_policy import seam_staging_want_mib

        wants = [seam_staging_want_mib(self._cfg_seam(c))
                 for c in (128, 256, 512, 1024, 2048, 4096)]
        self.assertEqual(wants, sorted(wants))

    def test_the_validation_prediction_is_on_record(self):
        """1024 was never measured; this is the number the boot must match."""
        from sglang.srt.managers.phase_policy import seam_staging_want_mib

        self.assertAlmostEqual(
            seam_staging_want_mib(self._cfg_seam(1024)), 1240.4, places=1
        )
