...
class TestGateExitsAreNamed(CustomTestCase):
    ...
    def test_the_three_classes_partition_every_exit(self):
        """MUTANT KILLED: fold NOT_APPLICABLE back into INERT.

        The coordinator's requirement for this slice, and the specimen's own
        shape: a healthy gate with nothing to do and a gate that is not there
        must not share a state. The three classes must also PARTITION -- no
        exit in two, none in neither -- or a caller is back to guessing.
        """
        every_exit = {
            REASON_CLEARED,
            REASON_SHORT,
        ...
        acted = {r for r in every_exit if r in ACTUATING_REASONS}
        idle = {r for r in every_exit if r in NOT_APPLICABLE_REASONS}
        defect = {r for r in every_exit if reason_is_defect(r)}
        self.assertEqual(acted | idle | defect, every_exit)
        self.assertEqual(acted & idle, set())
        self.assertEqual(acted & defect, set())
        self.assertEqual(idle & defect, set())
        # The specimen's exit is the idle one, NOT the defect one.
        self.assertIn(REASON_HEADROOM_SUFFICIENT, idle)
        self.assertNotIn(REASON_HEADROOM_SUFFICIENT, defect)
        # A missing mechanism is the defect one.
        self.assertIn(REASON_NO_GUARD, defect)

    def test_an_unclassified_exit_defaults_to_LOUD(self):
        """A reason nobody classified must shout, not hide.

        MUTANT KILLED: implement the defect class as a third explicit
        frozenset. Then a newly added exit belongs to no class and silently
        reads as not-a-defect -- which is the original bug, rebuilt.
        """
        self.assertTrue(reason_is_defect("some-exit-added-later"))
...
class TestOutcomesAreConsumed(CustomTestCase):
    ...
    def test_a_healthy_gate_reports_not_applicable_not_inert(self):
        clock = _Clock()
        sched = _FakeScheduler(queued=2, running=0, age=200.0)
        setattr(
            sched,
        ...
        self.assertEqual(channel.last_outcome.state, STATE_NOT_APPLICABLE)
        self.assertEqual(channel.last_outcome.reason, REASON_HEADROOM_SUFFICIENT)

    def test_a_missing_mechanism_reports_INERT_not_not_applicable(self):
        """The other half of the split: no guard at all is a DEFECT.

        MUTANT KILLED: classify every non-actuating exit as NOT_APPLICABLE.
        Then the state that looks identical to health from the outside -- the
        gate never built -- goes quiet, which is the failure this whole file
        is named after.
        """
        clock = _Clock()
        sched = _FakeScheduler(queued=2, running=0, age=200.0)
        setattr(sched, "phase_flip_corridor_admission", _gate(None))
        driver = self._driver(sched, clock, grace_s=45.0, retry_s=30.0)
        ...
        self.assertEqual(outcome.reason, REASON_NO_GUARD)
        self.assertNotEqual(outcome.state, STATE_NOT_APPLICABLE)

    def test_a_run_of_not_applicable_still_escalates(self):
        """Quiet per attempt, loud as a run.

        NOT_APPLICABLE is not a defect of the gate, so a single one must not
        shout -- but as an answer to "is this wedge recovering" it is a no,
        and a run of them is the finding.

        MUTANT KILLED: drop NOT_APPLICABLE from NON_ACTUATING_STATES to make
        it "harmless". The escalation then never fires on the exact shape the
        2026-08-22 specimen was in.
        """
        clock = _Clock()
        sched = _FakeScheduler(queued=2, running=0, age=200.0)
        setattr(
