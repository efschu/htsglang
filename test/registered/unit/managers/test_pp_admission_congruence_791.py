...
class UnhonourableCaseDegradesSafelyTest(CustomTestCase):
    ...
    def test_local_below_told_is_excluded_not_corrupted(self):
        ...
        self.assertNotIn(
            "req",
            eff1,
            "an unhonourable rid must be ABSENT from effective, never mapped "
            "to a fabricated or truncated-to-told length -- a caller that "
            "defaults a missing rid to 0 gets full recompute, which is "
            "correct; a caller that defaults it to `told` would corrupt the "
            "tensor",
        )
        self.assertEqual(len(cm.output), 1, "exactly one WARNING, not a flood")
        msg = cm.output[0]
        self.assertIn("rank", msg.lower())
        self.assertIn("1", msg)  # rank=PP1
        self.assertIn("req", msg)
        self.assertIn("120", msg)  # told
        self.assertIn("64", msg)  # local
        self.assertNotIn("ERROR", msg)
        self.assertNotIn("CRITICAL", msg)

        entry1 = decision1.by_rid()["req"]
        self.assertTrue(entry1.retracted)
        self.assertFalse(entry1.admitted)
        self.assertEqual(entry1.retracted_by_rank, PP1)

    def test_the_retraction_is_carried_forward_not_relogged(self):
        """PP2 must see the SAME membership verdict PP1 reached, and must
        not re-evaluate or re-warn about a decision that is no longer live."""
        decision0 = _decision(req=(120, 80))
        _eff1, decision1 = reconcile_pp_admission_decision(
            decision0, {"req": 64}, rank=PP1, pp_size=WORLD
        )

        logger = logging.getLogger("sglang.srt.managers.pp_admission_congruence")
        records = []
        handler = logging.Handler()
        handler.emit = records.append  # type: ignore[assignment]
        logger.addHandler(handler)
        try:
            # PP2's own local match is irrelevant here on purpose: even a
            # rank that WOULD have honoured told must not resurrect an
            # already-retracted rid.
            eff2, decision2 = reconcile_pp_admission_decision(
                decision1, {"req": 999}, rank=PP2, pp_size=WORLD
            )
        finally:
            logger.removeHandler(handler)

        self.assertEqual(records, [], "an already-retracted rid must not be re-logged")
        self.assertNotIn("req", eff2, "PP2 must agree with PP1's retraction")
        entry2 = decision2.by_rid()["req"]
        self.assertTrue(entry2.retracted)
        self.assertEqual(
            entry2.retracted_by_rank,
            PP1,
            "the retraction records WHO raised it, unchanged by later ranks",
        )
    ...
    def test_a_sibling_request_in_the_same_decision_is_unaffected(self):
        ...
        self.assertEqual(len(cm.output), 1, "only the bad rid is worth a warning")
        self.assertNotIn("bad", eff1)
        self.assertEqual(eff1, {"good": 30})
        self.assertFalse(decision1.by_rid()["good"].retracted)
