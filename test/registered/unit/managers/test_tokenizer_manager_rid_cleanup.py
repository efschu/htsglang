...
Covers:
  ...
  - Handler failures clean up pending and dispatched requests (#35255)
  - A waiter built before its request finishes still delivers (#36638)
"""
...
class TestReleaseReqStatesOnFailure(CustomTestCase):
    """Direct tests for _release_req_states_on_failure (upstream #35255)."""
    ...
    def test_dispatched_single_is_aborted_and_state_kept(self):
        ...
        # kept until the scheduler's abort answer removes it
        self.assertIn(rid, tm.rid_to_state)
...
class TestParallelSamplingFailureCleansGeneratedRid(CustomTestCase):
    """The rids _handle_batch_request mints for n>1 are released too (#35255)."""
...
class TestDisconnectAfterDispatchAbortsRequest(CustomTestCase):
    """Cancellation after dispatch must stop the scheduler request (#35255).

    On this line the handler caught only Exception, so a client disconnect
    (CancelledError) skipped the cleanup altogether and the scheduler request
    ran to its end holding its KV; the same leak followed any post-dispatch
    Exception, whose cleanup dropped the state so the later abort was
    swallowed as an unknown rid.
    """
    ...
    def test_cancel_after_dispatch_sends_abort_and_keeps_state(self, _wrap_shm):
        ...
        obj.stream = False
        obj.background = True  # no fastapi disconnect probe in the waiter
        tokenized = MagicMock()
        tokenized.rid = rid
        tokenized.mm_inputs = None
        tm._tokenize_one_request = AsyncMock(return_value=tokenized)
...
class TestWaitOneResponseAfterStateFreed(CustomTestCase):
    """A waiter built before its request finishes must still deliver (#36638).
