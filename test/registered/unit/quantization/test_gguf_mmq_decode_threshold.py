...
def test_decode_bucket_registration_happens_after_the_last_capture_bs_edit():
    """The published buckets must be the FINAL captured set.

    The threshold decides per enclosing bucket; on replay the token count IS a
    captured bucket, so the rounding is the identity and the replayed kernel
    matches the captured one. That argument only holds if what was published
    equals what was captured.

    Two later passes in DecodeCudaGraphRunner.__init__ narrow `capture_bs`
    (the MoE-offload cap and the weightless-KV block-graph cap). Publishing
    before them is correct only by accident of direction: shrinking leaves a
    superset, which still contains every replayed bucket. Anything that ADDS a
    bucket after publication would let a captured graph replay a different
    kernel than it was captured with -- silently, with no crash and no wrong
    shape. Pin the order so that cannot be reintroduced.
    """
    import ast
    import pathlib
    ...
    cls = next(
        ...
        if isinstance(n, ast.ClassDef) and n.name == "DecodeCudaGraphRunner"
    )
    init = next(
        n for n in cls.body
    ...
    for node in ast.walk(init):
        if isinstance(node, ast.Assign):
            for t in node.targets:
                if (isinstance(t, ast.Attribute) and t.attr == "capture_bs"
                        and isinstance(t.value, ast.Name) and t.value.id == "self"):
                    last_assign = max(last_assign or 0, node.lineno)
        if (isinstance(node, ast.Call) and isinstance(node.func, ast.Name)
                and node.func.id == "_register_gguf_decode_buckets"):
            register = node.lineno

    assert register is not None, "the decode-bucket registration disappeared"
    assert last_assign is not None, "self.capture_bs is no longer assigned"
    assert register > last_assign, (
        f"_register_gguf_decode_buckets runs at line {register}, before the "
        f"last self.capture_bs assignment at line {last_assign}: the published "
