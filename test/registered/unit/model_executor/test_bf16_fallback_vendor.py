...
class TestBf16FallbackIsVendorFirst(unittest.TestCase):
    def _run(self, *, hip, capability=(9, 0), bf16=None):
    ...
    def test_gfx900_without_bf16_falls_back_despite_reporting_9_0(self):
        """(9, 0) must not be read as 'Hopper, so bf16 is fine'."""
        self.assertTrue(self._run(hip="6.3.0", capability=(9, 0), bf16=False))

    def test_amd_with_bf16_keeps_bfloat16(self):
        """MI300-class: has bf16, so no fallback -- the probe must not
        over-trigger and silently downcast a capable card."""
        self.assertFalse(self._run(hip="6.3.0", capability=(9, 4), bf16=True))
    ...
    def test_amd_probe_failure_is_not_fatal(self):
        ...
        with self.assertLogs(mr.logger, level="WARNING") as logs:
            self.assertFalse(self._run(hip="6.3.0", capability=(9, 0)))
    ...
    def test_cuda_never_consults_the_functional_probe(self):
