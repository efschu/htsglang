...
class TestBaseReasoningFormatDetector(CustomTestCase):
    ...
    def test_finish_flushes_truncated_reasoning_no_stream_reasoning(self):
        """Bug regression (#32225): with stream_reasoning=False the base detector
        buffers the whole thinking block and only emits it on </think>. A stream
        cut short (e.g. max_tokens) before </think> left the trace stuck in
        _buffer, and nothing flushed it. finish() must emit it as reasoning,
        with the opening think token stripped, matching the non-streaming path."""
        detector = BaseReasoningFormatDetector(
            "<think>", "</think>", stream_reasoning=False
        )
        detector.parse_streaming_increment("<think>")
    ...
    def test_finish_after_closed_reasoning_is_noop(self):
        """Once </think> closed the block, finish() emits nothing."""
        detector = BaseReasoningFormatDetector(
            "<think>", "</think>", stream_reasoning=False
        )
        detector.parse_streaming_increment("<think>r</think>answer")
        end = detector.finish()
        self.assertEqual(end.reasoning_text, "")
        self.assertEqual(end.normal_text, "")
...
class TestDeepSeekR1Detector(CustomTestCase):
    ...
    def test_finish_flushes_truncated_forced_reasoning(self):
        """Bug regression (#32225): DeepSeek-R1 forces reasoning without a <think>
        start token, so the whole output is reasoning until </think>. With
...
class TestQwen3Detector(CustomTestCase):
    ...
    def test_finish_flushes_truncated_thinking_27b_form(self):
        """27B serving form (#32225): qwen3 parser, thinking on (force_reasoning,
        the template opens <think> in the prompt), stream_reasoning=False, stream
        cut by max_tokens before </think>: the trace must reach the client via
        ReasoningParser.parse_stream_end()."""
        rp = ReasoningParser("qwen3", stream_reasoning=False, force_reasoning=True)
        self.assertEqual(rp.parse_stream_chunk("step one, "), ("", ""))
        self.assertEqual(rp.parse_stream_chunk("step two"), ("", ""))
        self.assertEqual(rp.parse_stream_end(), ("step one, step two", ""))
        self.assertEqual(rp.parse_stream_end(), ("", ""))

    def test_finish_no_duplicate_when_streaming_reasoning(self):
        """stream_reasoning=True (default): reasoning already went out chunk by
        chunk; the stream-end flush must not repeat it."""
        rp = ReasoningParser("qwen3", stream_reasoning=True, force_reasoning=True)
        self.assertEqual(rp.parse_stream_chunk("step one"), ("step one", ""))
        self.assertEqual(rp.parse_stream_end(), ("", ""))
...
class TestCohereCommand4DetectorFinish(CustomTestCase):
    """finish() flush for Cohere's custom streaming state machine (#32225).
