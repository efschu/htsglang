...
"""barlink gegen NCCL auf der KOLLEKTIV-Ebene, verschraenkt im selben Lauf.
...
sind die beiden Wege, die im Betrieb wirklich laufen: barlink (der eigene
...
BACKENDS = ("nccl", "barlink:device", "barlink:shm")
...
def run_rank(a: argparse.Namespace) -> int:
    ...
    # Getrennte Gruppen: barlink braucht eine reine gloo-CPU-Gruppe, NCCL
    ...
    for name in ("device", "shm"):
        ...
        try:
            ...
            want = {"device": "BarlinkDeviceTransport", "shm": "BarlinkShmTransport"}[name]
            if got != want:
                err = (
                    f"Transport nicht aktiv (aktiv: {got or 'gloo-Inline-Ebene'}); "
                    f"barlink.py faellt bei Aufbaufehlern still auf gloo zurueck"
                )
