...
OUT="${OUT:-/spinning/gpu-battery-results/2026-08-01_370_sm120_configs}"
...
CN="a370_$ARM"
H_LOG="/root/battery-bar1/370d.$ARM.log"
...
echo "=== $(date -u +%H:%M:%SZ) arm=$ARM fmt=$FMT transport=$TRANSPORT regime=$REGIME model=$MODEL_SUB mlp=$MLP reserve=$RESERVE ==="

# In-container launch command. --network host so the host loopback reaches it.
# REGIME decides capture. eager -> --disable-cuda-graph, no capture at all.
...
LAUNCH="cd /wt && $INJECT python3 -m sglang.launch_server \
  ...
  --max-running-requests 16 $GRAPH_FLAG \
...
hssh 30 "setsid bash -c 'docker logs -f $CN > $H_LOG 2>&1' </dev/null >/dev/null 2>&1 & echo \$! > /root/battery-bar1/370d.$ARM.logpid; echo logfollower \$(cat /root/battery-bar1/370d.$ARM.logpid)" || true
...
# POINTS=0 makes this an EVIDENCE-ONLY boot: gate + boot-to-ready + the config
# -miss inventory, no measurement. Used for the INT8 graph boot in #370, where
# the format comparison is gated on both formats being tuned first, so taking
# INT8 numbers now would produce a figure nobody may compare to anything.
if [ "${POINTS:-1}" = "0" ]; then
    echo "arm=$ARM POINTS=0 -- evidence-only boot, skipping measurement"
    hssh 60 "grep -oE 'configs/N=[0-9]+,K=[0-9]+,device_name=[A-Za-z_0-9]+,dtype=[a-z0-9_]+,block_shape=\[[0-9, ]+\].json' $H_LOG | sort -u" \
        > "$OUT/config_miss_$ARM.txt" 2>&1
    echo "--- config-miss inventory ($(wc -l < "$OUT/config_miss_$ARM.txt") shapes) ---"; cat "$OUT/config_miss_$ARM.txt"
    hssh 60 "nvidia-smi --query-gpu=index,name,memory.total,memory.used,memory.free --format=csv,noheader" > "$OUT/vram_$ARM.txt" 2>&1
    hssh 120 "kill \$(cat /root/battery-bar1/370d.$ARM.logpid 2>/dev/null) 2>/dev/null; docker stop -t 20 $CN >/dev/null 2>&1; docker rm -f $CN >/dev/null 2>&1 || true"
    echo "arm=$ARM DONE"
    exit 0
fi
for N in 1 8; do
...
hssh 120 "kill \$(cat /root/battery-bar1/370d.$ARM.logpid 2>/dev/null) 2>/dev/null; \
