...
# Floor under every prompt's own max_new_tokens. The point driver reads this as
# an ENVIRONMENT variable of its own (S16_MIN_MAX_NEW_TOKENS), and ssh does not
# carry the caller's environment, so it has to be baked into the generated
# remote_measure.sh below. Without that the caller exports the floor, the host
# never sees it, and the run silently measures the unfloored working point.
# 0 = the prompt file's own budgets, unchanged. See s16_structured_point.py.
MIN_MAX_NEW="${S16_MIN_MAX_NEW_TOKENS:-0}"
...
# GENERATED by s16_dflash_structured.sh. Runs on the PVE host.
set -uo pipefail
S16_MIN_MAX_NEW_TOKENS=$MIN_MAX_NEW \\
