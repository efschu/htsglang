...
PYEOF
  ...
  echo "     spill depth 'arena' on this boot (selectable: none | draft |"
  echo "     arena). The flag that set it was removed by #1233 (WEG 2);"
  echo "     this line records the boot, not a knob to pass. Arena tail"
  echo "     releases: $(c 'rung 3 released')."
  ...
  echo "     purity 'strict' (pre-#1233 flag, removed with the flip);"
  echo "     prefill-batches-with-a-graph must be"
  ...
  echo "     15a spill-before-alloc: enforced at the seam ($GATE_OK clears) AND,"
  echo "         new this shift, at prefill admission ($PREF_ARM arms), which"
  echo "         closes register C17's 'one site only'."
  if [ "${PREF_ARM:-0}" -eq 0 ]; then
    echo "         THE 0 IS NOT A FAILURE AND NOT A PASS -- read it exactly."
    echo "         The gate is proven INSTALLED by the ARMED line quoted above;"
    echo "         0 arms means the corridor was never under the arming floor"
    echo "         AT AN ADMISSION INSTANT. On this boot the deep troughs are"
    echo "         intra-forward transients (memory taken and returned inside"
    echo "         one chunk's execution) and flip-seam moments where strict"
    echo "         purity has already parked prefill -- neither is reachable"
    echo "         from a scheduling boundary. What this gate closes is the"
    echo "         SUSTAINED under-floor state s33 measured for 1.6 s; it does"
    echo "         not claim the transient."
  fi
