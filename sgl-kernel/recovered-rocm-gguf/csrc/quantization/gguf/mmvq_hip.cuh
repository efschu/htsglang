...
static void mul_mat_vec_q_dispatch(
    const void* vx, const void* vy, scalar_t* dst, const int ncols, const int nrows, const int nvecs, hipStream_t stream) {
...
#define SGLANG_MMVQ_NC_CASE(NC)                                                 \
  case NC:                                                                      \
   hipLaunchKernelGGL(( mul_mat_vec_q_nc<scalar_t, NC, qk, qi, block_q_t, vdr, vec_dot_q_cuda>)      \
        , dim3(block_nums_nc), dim3(block_dims), 0, stream, vx, vy, dst, ncols, nrows);  \
    break;
...
#undef SGLANG_MMVQ_NC_CASE
    default: {
     ...
     hipLaunchKernelGGL(( mul_mat_vec_q<scalar_t, qk, qi, block_q_t, vdr, vec_dot_q_cuda>)
...
#define SGLANG_MMVQ_KQ_CASE(NC)                                                                 \
  case NC: {                                                                                    \
   ...
   hipLaunchKernelGGL(( mul_mat_vec_q_nc_kq<scalar_t, NC, qk, qi, block_q_t, vdr, vec_dot_q_cuda>)                   \
