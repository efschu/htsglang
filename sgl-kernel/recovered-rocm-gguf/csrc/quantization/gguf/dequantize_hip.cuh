...
template <int qk, int qr, dequantize_kernel_t dequantize_kernel, typename dst_t>
static void
dequantize_block_cuda(const void* __restrict__ vx, dst_t* __restrict__ y, const int k, hipStream_t stream) {
...
static void dequantize_row_mxfp4_cuda(const void* vx, dst_t* y, const int k, hipStream_t stream) {
  const int nb = (k + QK_K - 1) / QK_K;
 hipLaunchKernelGGL(( dequantize_block_mxfp4), dim3(nb), dim3(32), 0, stream, vx, y, k / QK_MXFP4);
}
