...
def swap_tag_pool(tag: str) -> Tuple[int, float]:
    """Move ``tag``'s LIVE tensors into a FRESH pool and drop the old one.

    Returns ``(moved_tensors, freed_gib)``.

    THE ONE THING THAT ACTUALLY RELEASES A PRIVATE POOL is dropping the last
    reference to it, and this is the only moment in a boot when that is
    cheap.  Measured on fnFL2v55 (``WEG2-XCHG-COVER``, the line that had the
    answer all along):

        rank   planned+walked under `weights`   saver booked   delta
        PP1            12.5 MiB                   9528 MiB    9.29 GiB
        PP2           628.2 MiB                   8726 MiB    7.91 GiB

    and the delta matches the per-rank load overhead (10.09 / 7.57 GiB) to
    0.8 GiB.  Those bytes are the SHARD LOAD: ``load_weights`` runs in the
    BASE tag -- ``weight_chunk_scope`` wraps only the post-load pass
    (model_loader/loader.py:971) -- and the presplit frees the originals
    python-side while their blocks stay in the base tag's MemPool, which
    ``empty_cache`` cannot reach and ``_cuda_releasePool`` will not release
    while the cache holds a reference (v52: "pools=6 released=0
    skipped_in_use=6").

    So: after the post-load pass, what still LIVES under the base tag is
    small and named (PP2: 628 MiB, essentially ``lm_head``).  Copy it into a
    new pool, drop the old pool object, and the dead shard blocks go back to
    the driver with it.

    Called with the base tag's scope ALREADY CLOSED -- with it open the new
    allocations would route into the pool being replaced.
    """
    import torch

    old = _TAG_MEM_POOLS.get(tag)
    if old is None or not torch.cuda.is_available():
        return 0, 0.0
    before = torch.cuda.memory_reserved()
    holders = _TAG_LIVE_TENSOR_HOLDERS.get(tag) or []
    moved = 0
    fresh_pool = None
    mempool_cls = getattr(torch.cuda, "MemPool", None)
    use_mem_pool = getattr(torch.cuda, "use_mem_pool", None)
    if mempool_cls is None or use_mem_pool is None:
        return 0, 0.0
    fresh_pool = mempool_cls()
    with torch.no_grad(), use_mem_pool(fresh_pool):
        for get, put in holders:
            data = get()
            if data is None or not data.is_cuda or data.numel() == 0:
                continue
            fresh = torch.empty_like(data)
            fresh.copy_(data)
            put(fresh)
            moved += 1
    _TAG_MEM_POOLS[tag] = fresh_pool
    del old
    import gc as _gc

    _gc.collect()
    torch.cuda.empty_cache()
    freed = (before - torch.cuda.memory_reserved()) / (1024 ** 3)
    logger.info(
        "WEG2-TAG-POOL swap tag=%s moved=%d freed_gib=%.2f -- the base tag's "
        "dead shard-load blocks go back with the old pool object",
        tag, moved, freed,
    )
    return moved, freed


#: ``tag -> [(getter, setter), ...]`` for the tensors a tag must KEEP when
#: its pool is swapped.  Registered by the one caller that knows them (the
#: model runner, right after the post-load pass), because the saver cannot
#: walk a model it does not own.
_TAG_LIVE_TENSOR_HOLDERS: Dict[str, list] = {}


def register_tag_live_tensors(tag: str, holders) -> None:
    """Declare which tensors :func:`swap_tag_pool` must carry across."""
    _TAG_LIVE_TENSOR_HOLDERS[tag] = list(holders)
