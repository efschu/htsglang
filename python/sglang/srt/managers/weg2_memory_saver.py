...
#: module global rather than a parameter because the step that needs it --
#: the MoE repack inside ``process_weights_after_loading`` -- is three frames
#: below the scope and reachable from quant methods this tree does not own.
...
def outside_tag_pool(reason: str = "") -> Iterator[bool]:
    ...
    The load transients (the Marlin repack's [E] stacks) must not be born in
    the tag pool: a private ``MemPool`` never hands cached blocks back while
    it lives, ``empty_cache`` does not reach into it, and the pool is cached
    per tag so it outlives the whole load.  Measured cost of letting them in
    (fnFL2v47/v48): 0.84 GiB per layer, pinned to the end of the boot.

    ``release_active_tag_pools`` is NOT an alternative to this: v52 proved it
    cannot fire, because ``MemPool.use_count()`` counts the reference the tag
    cache itself holds, so the allocator refuses to release a pool that is
    merely cached ("pools=6 released=0 skipped_in_use=6 freed_gib=0.00").

    Yields True when it really stepped out.  Everything allocated inside is
    in the DEFAULT pool and therefore in segments that carry no tag, so the
    caller MUST move anything that has to survive back in
    (:func:`copy_into_tag_pool`) before leaving.
    """
    import torch
...
def copy_into_tag_pool(module) -> Tuple[int, float]:
    """Re-allocate ``module``'s CUDA parameters INSIDE the active tag pool.

    Returns ``(count, gib)``.  The point of the tag pool is segment purity
    (#1378 xsn66: a segment carries the tag it was opened under), so a tensor
    produced outside it has to be copied back before the scope ends -- else
    the exchange pauses a segment that holds another tag's bytes.

    The copy is device-to-device and costs about 2 ms per GiB on this rig,
    against 0.84 GiB per layer of pinned transients it removes.
    """
    import torch

    if _ACTIVE_TAG_POOL is None:
        return 0, 0.0
    n = 0
    moved = 0
    with torch.no_grad():
        for name, param in list(module.named_parameters(recurse=True)):
            ...
            param.data = fresh
            n += 1
            moved += data.numel() * data.element_size()
    return n, moved / (1024 ** 3)
