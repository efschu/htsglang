...
class Scheduler(
    ...
    def _update_uniform_pool_budget(self) -> None:
        ...
        # #1028g PHASE 1 -- MEASURE-ONLY, AND IT MUST NOT STAY THAT WAY.
        #
        # WHY THIS SITE. The 2026-08-30 stalls put all three ranks' main-thread
        # time in collectives (all_reduce 30-35 %, broadcast 4-27 %) while a
        # healthy window under the same load shows NO collective in the top 7.
        # The ranks are not slow; they are in DIFFERENT collectives and none
        # can finish. This is the gloo `tp_cpu_group`, which the #650/#653
        # barlink coverage never reached -- no deadline, no peer positions.
        #
        # WHY NOT A TIMEOUT ON THE WORK HANDLE. #630's lesson is to DRIVE
        # wait(), never poll is_completed(); but an async handle whose
        # wait(timeout) expires is in an undefined state, so a "report-only"
        # timeout would silently stop being semantics-preserving -- the
        # opposite of a measurement. Instead this PUBLISHES a timestamp and
        # measures the completed wait, exactly the pattern the chain receive
        # already uses (`on_blocked_recv`, request_receiver.py:89-95). No
        # timeout, no new collective, no behaviour change.
        #
        # PROMOTION CRITERION -- this comment is the anti-#578 clause, because
        # an instrument that is armed and never promoted is the failure this
        # corpus keeps recording. Phase 2 replaces the WARNING below with
        # CRASH/STOP plus the same census dump, and may ONLY be promoted when
        # BOTH hold:
        #   (i)  zero waits above the chosen deadline across a full healthy
        #        pack-4 -> pack-5 sequence, and
        #   (ii) one caught stall in which the report path below demonstrably
        #        fired and produced both ranks' census history.
        # The deadline is then max(measured_max * 2, remaining build window)
        ...
        try:
            torch.distributed.all_reduce(
                t, op=torch.distributed.ReduceOp.MIN, group=grp
            )
        finally:
        ...
        if _cw_waited > 30.0:
            ...
            # cannot flood.
            logger.warning(
                "#1028g UNIFORM-REDUCE SLOW: rank %s waited %.1fs in the gloo "
                "tp_cpu_group all_reduce (measure-only, no deadline armed). "
                "This is a DISTRIBUTION SAMPLE for the phase-2 deadline.",
                getattr(getattr(self, "ps", None), "pp_rank", "?"),
                _cw_waited,
            )
            if _cw_waited > 300.0:
                # Upper-bound catcher: at this point the wait completed, but it
                # took long enough that the group was effectively dead. Dump
                # the COLLECTIVE-FREE census (collective_census.format_local_
                # census takes no collective by design, which is the whole
                # reason it exists) so the family sequences can be diffed
                # against the peers' afterwards.
                try:
                    from sglang.srt.distributed import collective_census

                    logger.warning(
                        "#1028g CENSUS DUMP after a %.0fs reduce: %s",
                        _cw_waited,
                        collective_census.format_local_history(
                            int(getattr(getattr(self, "ps", None), "pp_rank", -1))
                        ),
                    )
                except Exception as e:  # noqa: BLE001 - a probe never kills
                    logger.warning("#1028g census dump unavailable: %s", e)
