    ...
    def _settle(self, channel) -> RecoveryOutcome:
        ...
        if outcome.state == STATE_UNCONSUMED:
            ...
            logger.error(
                ...
                "consumed it after %.1fs and the relief ladder did nothing, "
                "exit '%s'. This names WHICH of the gate's exits was taken; "
                "it is not the old 'returned None', which six different exits "
                "produced and which was read as 'the gate is off' on a boot "
                "where the gate was demonstrably armed.",
                ADMISSION_WEDGE,
                outcome.seq,
                outcome.waited_s,
                outcome.reason,
