    ...
    def _weg2_shadow_hook(self, hook: str, *, recv_req, reserve_bytes: int = 0,
        ...
        try:
            ...
            if group not in ("P", "D") or rank < 0:
                ...
                # it is the same W81 line every other rank-local refusal on
            ...
            # ``XchgRegion.begin_flip``, which refused it as **W70
            ...
            # exists yet -- the W81 printed ``epoch=`` EMPTY, not malformed --
            ...
            # is missing becomes a NAMED W81 refusal instead of a sentinel that
            # downstream code has to recognise.  The sentinels that used to
            # travel were: leg=-1 / epoch="" (W70, above), card_uuid="unknown"
            ...
            if not card_uuid:
                logger.info(sh.rank_local_skip_message(
                    ...
                    detail=f"hook={hook} -- NVML could not name this rank's "
                           ...
                           f"deposit charge, the W79 line) is keyed by it"))
                return
