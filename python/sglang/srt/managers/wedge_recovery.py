...
#: The scheduler thread consumed the request and the ladder did nothing. The
#: outcome carries WHICH exit -- see ``corridor_admission``'s REASON_* set.
STATE_INERT = "INERT"
...
#: States in which the recovery demonstrably failed to change anything. A
#: run of these is what escalation counts.
NON_ACTUATING_STATES = frozenset({STATE_INERT, STATE_UNCONSUMED})
...
class WedgeRecoveryChannel:
    ...
    def settle(
        ...
        if self.acked_seq >= seq:
            ...
            state = STATE_ACTUATED if reason in ACTUATING_REASONS else STATE_INERT
            return RecoveryOutcome(state, reason, seq, waited)
        if waited < grace_s:
    ...
    def drain(self, scheduler: Any) -> Optional[RecoveryOutcome]:
        ...
        return RecoveryOutcome(
            STATE_ACTUATED if reason in ACTUATING_REASONS else STATE_INERT,
            reason,
            seq,
            0.0,
        )
