...
def step_reference(tables: PoolTables, ids, buffers: StepBuffers) -> Tuple[List[Tuple[int, int]], Any]:
    ...
    for e in selected:
        ...
        if gate:
            miss_count[e] += 1
        victim = -1
        if promote_ok and (limit == 0 or len(gathers) < limit) and miss_count[e] >= min_misses:
            best = None
    ...
    def pool_step(
        ...
        for i in range(0, WIDTH):
            ...
            if is_miss > 0:
                ...
                if promote_ok & ((limit == 0) | (promoted < limit)) & (misses_so_far >= min_misses):
                    best = tl.min(use, 0)
                    if best != never:
