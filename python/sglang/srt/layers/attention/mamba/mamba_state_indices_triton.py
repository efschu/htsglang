...
Ported from upstream sglang #32219 ("[MTP] Cut spec-v2 host-seam overhead in
hybrid-linear MTP decode"); the fork keeps its mamba Triton ops under
``srt/layers/attention/mamba`` rather than ``kernels/ops/mamba``.

``MambaAttnBackendBase._replay_metadata`` refreshes the captured per-bs
``state_indices_list`` buffer before every cuda-graph replay (decode, and on
the 27B's D group the DFLASH target-verify graph every step). The reference
form is a chain of dispatched aten ops whose host cost sits in the bs=1
inter-phase seam:
