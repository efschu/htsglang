"""#37500 port: DSA/QSA index top-k sharing across MTP draft iterations.

Upstream (`sgl-project/sglang` #28609, 2026-08-06) ships this next to a
context-parallel (CP) gather; this line has no CP, so the CP hooks resolve
to no-ops. Everything else is verbatim upstream.
"""
...
try:  # CP is not on this line; keep the upstream call shape.
    from sglang.srt.layers.cp.utils import cp_gather_after_forward, is_cp_active
except ImportError:  # pragma: no cover - exercised on this line

    def is_cp_active(forward_batch) -> bool:  # type: ignore[misc]
        return False

    def cp_gather_after_forward(topk_indices, forward_batch):  # type: ignore[misc]
        return topk_indices
