...
def _fused_qk_rmsnorm_rope_gate_kernel(
    ...
    # Upstream #34446: under mrope the positions are [3, T] (temporal, height,
    # width) and each rotary lane takes its row from the axis map. Reading only
    # ``positions_ptr + token`` (row 0) rotated every image token by its
    # temporal position alone -- right for text (t == h == w), wrong for vision.
    if MROPE:
