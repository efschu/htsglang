...
#: group P through ``spec_form_env("P")``; UNIFY S6: the ``p_draft`` axis
#: ``cold`` = 0, ``compute`` = 1). See environ.py.
DFLASH_PRODUCE_ENV = "SGLANG_WEG2_DFLASH_PRODUCE"


def dflash_produce_on_p() -> bool:
    """Does group P's DFlash draft-KV producer COMPUTE (27B user decision
    2026-09-24: default off in the launcher, ``p_draft=cold``)?
