...
class DraftBlockProposer:
    ...
    def _fill_dp_moe_sync_metadata(
        ...
        # #33098: the EP token-accounting path reads num_token_non_padded /
        # _cpu (layers/moe/topk.py, hash_topk.py, mega_moe.py), not the global
        # vectors above. Without them a DSpark draft forward under DP+EP
        # accounts its tokens against a stale or absent count.
