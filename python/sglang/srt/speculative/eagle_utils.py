...
def _seeded_verify_coins(
    *,
    ...
    Mirrors the main seeded-sampling path (Sampler._sample_from_logprobs):
    murmur_hash32(seed, seq_lens, column) mapped to [0, 1). Columns
    [0, draft_token_num) drive the per-draft rejection coins; column
    draft_token_num drives the final fallback-sampling coin.

    Axis this covers (port of upstream sgl-project/sglang#30822): RUN-TO-RUN
    reproducibility -- the same pinned seed replays the same accept decisions
    across separate server starts. It is orthogonal to the fork's #50 fix,
    which broadcasts the verify OUTCOME from rank 0 further down in
    eagle_sample and gives CROSS-RANK agreement inside one heterogeneous-TP
    run. The coins are drawn before that broadcast and do not displace it:
    seeding makes each rank's draw identical, broadcasting makes the accepted
    result identical even when the surrounding float math is not.

    Scope: this seeds only the verify-side RNG. With rejection sampling the
    draft workers still pick candidates via unseeded multinomial (fast_sample
    in eagle_worker_v2), so that mode stays non-deterministic until the draft
    RNG is seeded in a follow-up; top-k/greedy draft selection is already
    deterministic.
    """
...
def _verify_coins(
    *,
    ...
    otherwise. Default (no --sampling-seed / no per-request seed) is the
    unchanged torch.rand path.
    """
...
def eagle_sample(
    ...
    if sampling_info.is_all_greedy or _is_cpu or _is_npu or _is_hip or _is_xpu:
        ...
        # Coins are drawn HERE, upstream of the rank-0 broadcast further down:
        # seeding fixes the draw per run, the broadcast fixes the outcome
        # across ranks. Both stay in force.
