...
#: --dflash-produce-on-p (user decision 2026-09-24, verbatim: "ne wir lassen
...
#: THE STANDARD FORM under --spec-form DFLASH is ``off``: group P still loads
...
#: growing linearly per 4096 chunk (393 ms @28k -> 1197 ms @94k). The switch
#: rides group P's ENVIRONMENT only (spec_form_env("P")) -- argv_p, the P FORM
...
_SPEC_FORM: Dict[str, object] = {
    ...
    # --draft-kv-on-p as RESOLVED by main (on, and HiCache enabled); the
    # CLI value until then. Read by p_group_has_draft_producer only.
    "draft_kv_on_p": DRAFT_KV_ON_P_DEFAULT == "on",
...
def apply_spec_form(ns) -> None:
    ...
    _SPEC_FORM["draft_kv_on_p"] = (
        str(getattr(ns, "draft_kv_on_p", DRAFT_KV_ON_P_DEFAULT) or DRAFT_KV_ON_P_DEFAULT).lower() == "on"
    )
...
def p_group_has_draft_producer() -> bool:
    """THE ONE detection of "does group P produce draft pages" (27B line),
    read off P's FORM -- never off whether D has a draft model (D always has
    one). True only when P carries the draft-KV producer (--draft-kv-on-p,
    resolved: on and HiCache enabled) AND that producer computes: under
    --spec-form DFLASH that is --dflash-produce-on-p on
    (SGLANG_WEG2_DFLASH_PRODUCE=1); the NEXTN producer always computes.
    Line-specific by design (the NF line's P form is --draft-kv-on-p off /
    SGLANG_WEG2_DRAFT_ON_P); a pick onto another line adapts only this."""
    if not bool(_SPEC_FORM.get("draft_kv_on_p", DRAFT_KV_ON_P_DEFAULT == "on")):
        return False
    if spec_form_is_dflash():
...
def resolve_hicache_draft_tier() -> Tuple[str, str]:
    ...
    if value == "off" and p_group_has_draft_producer():
        ...
        raise Weg2LaunchRefused(
            ...
            "produces draft pages (--dflash-produce-on-p on, or the NEXTN producer with "
            "--draft-kv-on-p on) -- the draft would get no HiCache space to publish into. "
...
def spec_form_env(group: str) -> Dict[str, str]:
    ...
    and (solo placement) the solo host's compact draft cache; P's
    draft-KV producer switch under DFLASH (--dflash-produce-on-p, ALWAYS
    written so the rank never falls back to its unset default)."""
...
def dflash_produce_line() -> str:
    ...
    if dflash_produce_on_p():
        return (
            "WEG2 DFLASH-PRODUCE-ON-P: on -- group P's DFlash draft-KV producer "
    ...
    return (
        "WEG2 DFLASH-PRODUCE-ON-P: off -- STANDARD FORM (user decision 2026-09-24): "
        "group P loads the DFlash2 draft on its last PP stage (cold-resident bytes, "
...
#: RC7-X: the name a negative --x-busy-tokens is refused with at launch.
...
def resolve_x_ceiling(ceiling: Optional[int], x_tokens: int,
    ...
    """RC7-X: group D's W50 riegel and the ``X CEILING:`` line -- ``(tokens, line)``.

    The NF line's H84 form and wording, so both lines log alike. D enforces
    law 4 for real, after ``match_prefix`` (argv_d ``--tp-prefill-max-tokens``).
    With the live X* re-solve working (r_D from D's own prefill time), the
    front's X rises above the launch X -- and every grant above the riegel D
    was given would come back as W50 and detour through P. So D's riegel is
    armed at ``--x-ceiling-tokens`` and the front clamps its live X to the same
    number (front ``--x-ceiling-tokens``).

    0 / unset = off: D's riegel and the front's X are exactly today's, the live
    X cannot rise. A ceiling below the launch X is LIFTED to it (H84; the line
    says so). A negative ``--x-busy-tokens`` is refused (W155, the 27B's own
    flag); 0 is legal and means no D prefill while D decodes others.
    """
    ...
    x = int(x_tokens)
    busy = ("4096 (front default, one chunk)" if x_busy is None else str(int(x_busy)))
    asked = int(ceiling or 0)
    if asked <= 0:
        return x, (f"X CEILING: off (--x-ceiling-tokens 0) -- group D --tp-prefill-max-tokens {x} "
                   f"= the front's start X, which is also the ceiling of the front's live X; "
                   f"27B X_busy={busy} while D decodes others")
    c = max(asked, x)
    lifted = "" if c == asked else f" (asked {asked} < start X {x}: lifted to the start X)"
    return c, (f"X CEILING: --x-ceiling-tokens {c}{lifted} -- group D --tp-prefill-max-tokens "
               f"{c} (its W50 riegel; D prefills in --chunked-prefill-size {CHUNKED_PREFILL_TOKENS} "
               f"chunks); front --tp-prefill-max-tokens {x} (start X, unchanged) --x-ceiling-tokens "
               f"{c}: the live X re-solves within [{CHUNKED_PREFILL_TOKENS}, {c}] from THIS boot's own "
               f"samples (r_D = D's own prefill time of a solo prefill); 27B X_busy={busy} while "
               f"D decodes others, X_idle = the live X only while D holds nothing (X-SOLO)")
...
def p_produces_draft_pages() -> bool:
    ...
    --dflash-produce-on-p off (the 2026-09-24 standard form)."""
...
def gate_w10(log_p: str, log_d: str, log, *, p_produces_draft_pages: bool) -> Dict[str, object]:
    """W10 (#1233) as a function so its skip is pinnable.

    Under --dflash-produce-on-p off group P registers its drafter (the
    producer is built) but writes no page under it, so D's draft reads miss
    whatever the identities say -- a mismatch there refuses a boot for a
    route that does not exist (metal: weg2xsn414 refused W10 with
    P=b53f8c336da51155 D=f0c73316257fec1d on the old producer form; the
    cause of that disagreement is noted, not chased -- the producer form is
    retired). The identities are still read and logged, never graded."""
...
def choose_host_ledger(
    ...
    #
    # NOT a second derivation of the size: `depth` is the SAME VALUE the helper
    # hands to `widest_layer_terms` -- since #1358 that is the `bounce_depth`
    # flag rather than the constant, and it is READ FROM THE SAME VARIABLE.
    # Leaving the constant here while the arm is priced at depth=1 would make
    # the ARM line print `bounce_depth=2` for a depth=1 arm: the field [23a]
    # introduced to stop a number having three readings would itself be the
    # lie. Caught at the merge by the train seat, who refused to resolve it
    # mechanically and was right to.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--x-ceiling-tokens", type=int, default=0,
                    help="RC7-X: the most the front's LIVE X (re-solved from this boot's own "
                         ...
                         "to it is not refused by D. 0 (default) = off = the launch X (today: D's "
                         "riegel and the front's X both at --tp-prefill-max-tokens, the live X "
                         "cannot rise; argv byte for byte as before). Below the launch X it is "
                         "lifted to the launch X (NF H84, same flag).")
    ...
    ap.add_argument(
        ...
        help="--spec-form DFLASH only (user decision 2026-09-24). 'off' (the default, "
             "the standard form): group P keeps loading the DFlash2 draft on its last "
             "PP stage -- the bytes stay cold-resident, so VRAM, the planner cut, the "
             ...
             "no draft pages (its existing cold path). 'on' is the old producer form, "
             "A/B only. The value reaches group P's ranks through its environment "
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    # (0 / unset = the launch X, today; below it lifted). W155 refuses a negative X_busy.
    ...
    log(x_ceiling_provenance)  # RC7-X: "X CEILING: ..." (the NF line's H84 line)
    ...
    log(f"SCHEDULING FLAGS AS EMITTED -- group P: --max-running-requests {p_bs} "
        ...
        + ("" if not ns.x_ceiling_tokens or int(ns.x_ceiling_tokens) <= 0 else f" --x-ceiling-tokens {x_d_riegel}")
    ...
    if not ring_table.p_carries_drafter(shipped_argv_p):
        ...
        # --dflash-produce-on-p off (2026-09-24): P builds the producer but
        # writes NO draft pages, so there is no identity for D to be matched
        # against -- W10 is skipped by name (the identities are still logged).
        # W11 still grades the cold-resident draft bytes on P's last stage.
...
def front_argv_for(py: str, store_dir: str, p_pid: int, d_pid: int, dc_expect_d: Dict[str, int],
    ...
    # (resolve_x_ceiling: 0 = off, below the start X lifted to it).
    if int(getattr(ns, "x_ceiling_tokens", 0) or 0) > 0:
        argv += ["--x-ceiling-tokens", str(max(int(ns.x_ceiling_tokens), int(x_tokens)))]
