...
THE TWO W-CODES ALLOCATED HERE are W70 and W71 (spec section 7).  Both were
...
RENUMBERED BY THE serve-next5 REPLAY (2026-09-09): W53/W54 here became W70/W71
...
merged maximum is W66 and W69-W84 are what the enumeration returns on it.
...
class Weg2XchgPlanDisagree(RuntimeError):
    """W70: two things that must agree about this exchange do not.
    ...
    **DEVIATION, stated: this code is used more widely than spec section 7's
    row for it.**  Section 7 assigns W70 to Gate 0 (asymmetric matrix, a
    ...
    region is*, which is the same refusal one layer down.  W72
...
class Weg2XchgGateTimeout(RuntimeError):
    """W71: a rank did not join a gate, or joined it and then died.
...
class XchgRegion:
        ...
        *,
        ...
        try:
            ...
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree create path={target}: a region for boot "
    ...
    def open(cls, path: str, *, expect_boot: str) -> "XchgRegion":
        ...
        def refuse(why: str) -> "Weg2XchgPlanDisagree":
            ...
            return Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree path={path}: {why}"
            )
    ...
    def begin_flip(self, flip_epoch: str) -> int:
        ...
        if boot != self.boot_nonce:
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree begin_flip epoch={flip_epoch!r}: its boot "
        ...
        if index <= self.flip_index:
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree begin_flip epoch={flip_epoch!r}: flip index "
    ...
    def _require_flip(self, what: str) -> None:
        if not self.epoch_hash:
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree {what}: no flip is bound to this view. "
    ...
    def publish(self, pair: int, slot: int, bytes_filled: int, *, checksum: int = 0) -> XchgSlot:
        ...
        if rec.state != SLOT_FILLING or rec.epoch_hash != self.epoch_hash:
            raise self._refuse(
                ...
                f"W70 Weg2XchgPlanDisagree publish pair={pair} slot={slot}: the slot is "
        ...
        if int(bytes_filled) > SLOT_BYTES:
            raise self._refuse(
                Weg2XchgPlanDisagree,
                f"W70 Weg2XchgPlanDisagree publish pair={pair} slot={slot}: "
    ...
    def claim_produced(
        ...
        * **FILLING at this epoch whose producer is gone from /proc** -> W71.
        ...
        if rec.state == SLOT_FILLING:
            if not _pid_alive(rec.producer_pid, proc_root):
                raise self._refuse(
                    Weg2XchgGateTimeout,
                    f"W71 Weg2XchgGateTimeout slot pair={pair} slot={slot} state=FILLING "
    ...
    while True:
        ...
        if monotonic() - started >= budget:
            ...
            raise region._refuse(
                Weg2XchgGateTimeout,
                f"W71 Weg2XchgGateTimeout gate=0 epoch={region.epoch} "
...
def gate0_check(
    ...
    *,
    ...
    """Check all 36 cells, every plan hash and every rank's verdict.  W70 on any gap.
    ...
    if not have_census and not census_unavailable_reason:
        raise region._refuse(
            Weg2XchgPlanDisagree,
            f"W70 Weg2XchgPlanDisagree gate=0 epoch={region.epoch} reader_row={row}: "
    ...
    if mismatches:
        raise region._refuse(
            Weg2XchgPlanDisagree,
            f"W70 Weg2XchgPlanDisagree gate=0 epoch={region.epoch} "
...
def wave_gate(
    ...
    *,
    ...
    """Close wave ``wave`` across all six ranks, or raise W71 naming who did not.
    ...
    is committed to the W75 roll-forward by a gate that reported PASS.
    ...
    def refuse(headline: str, which: Sequence[int], rows: Sequence[GateRow],
        ...
        return Weg2XchgGateTimeout(
            f"W71 Weg2XchgGateTimeout epoch={region.epoch} wave={wave} "
