...
* **SECTION S2 -- COVERAGE ARMING (W69) AND THE DRAFT TAG** (spec 6/S2, 4.1,
...
class Weg2XchgPlanDisagree(RuntimeError):
    """W70 Weg2XchgPlanDisagree -- the plan does not describe the hardware.
...
class Weg2XchgSourceMissing(RuntimeError):
    """W76 Weg2XchgSourceMissing -- a destination byte range has no VRAM source
...
class StorageGeom:
    ...
    def of(cls, tensor) -> StorageGeom:
        ...
        if len(shape) == 1:
            if stride[0] != 1:
                raise Weg2XchgPlanDisagree(
                    f"W70 Weg2XchgPlanDisagree: 1-D tensor with stride {stride} "
        ...
        if len(shape) > 2:
            ...
            if not flat:
                raise Weg2XchgPlanDisagree(
                    f"W70 Weg2XchgPlanDisagree: {len(shape)}-D tensor shape "
        ...
        if stride[1] == 1:
            ...
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree: tensor shape {shape} stride {stride} "
...
def shard_offsets(
    ...
    if tp_size <= 0:
        raise Weg2XchgPlanDisagree(
            f"W70 Weg2XchgPlanDisagree: tp_size {tp_size} is not a group size."
        )
    if ratios is not None and len(ratios) and len(ratios) != tp_size:
        raise Weg2XchgPlanDisagree(
            f"W70 Weg2XchgPlanDisagree: ratio vector {list(ratios)} has "
    ...
    if not ratios:
        if total % tp_size != 0:
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree: dimension {total} is not divisible "
    ...
    if acc != total:
        raise Weg2XchgPlanDisagree(
            f"W70 Weg2XchgPlanDisagree: per-rank sizes {sizes} sum to {acc}, "
...
class GroupLayout:
    ...
    def validate(self) -> None:
        ...
        if self.n_ranks <= 0:
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree: group {self.name!r} has no cards."
            )
        if self.tp_size not in (1, self.n_ranks):
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree: group {self.name!r} has tp_size "
                f"{self.tp_size} over {self.n_ranks} ranks. The V1 scope is "
                f"pure TP (tp_size == ranks) or the PP form (tp_size == 1)."
            )
        if int(self.base) < 0:
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree: group {self.name!r} base "
        ...
        for label, vec in vectors:
            if vec and len(vec) != self.tp_size:
                raise Weg2XchgPlanDisagree(
                    f"W70 Weg2XchgPlanDisagree: group {self.name!r} carries a "
...
class ParamGeom:
    ...
    between it and the pad is a GAP and is refused by name (W76), never zeroed.
    ...
    def validate(self) -> None:
        ...
        if self.rows_full <= 0 or self.cols_full <= 0 or self.itemsize <= 0:
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree: {self.name}: extents "
        ...
        if self.shard_axis not in (ROWS, COLS, REPLICATED):
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree: {self.name}: shard axis "
                f"{self.shard_axis} is neither ROWS, COLS nor REPLICATED."
            )
        if self.blocks:
            if self.shard_axis != ROWS:
                raise Weg2XchgPlanDisagree(
                    f"W70 Weg2XchgPlanDisagree: {self.name}: packed outputs "
            ...
            if any(int(b) <= 0 for b in self.blocks):
                raise Weg2XchgPlanDisagree(
                    f"W70 Weg2XchgPlanDisagree: {self.name}: packed output "
        ...
        if not 0 <= int(self.pad_units) <= self.shard_total:
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree: {self.name}: pad_units "
        ...
        if self.src_extent is not None and not (
            ...
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree: {self.name}: src_extent "
        ...
        if (
            ...
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree: {self.name}: seeded widths "
    ...
    def of(
        ...
        **kw,
        ...
        if ndim > 2 and (shard_dim is None or int(shard_dim) not in (0, ndim - 1)):
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree: {name} is {ndim}-D and its shard "
...
def plan_id(descs: Sequence[XchgDesc], waves: Sequence[Sequence[str]] = ()) -> str:
    ...
    whole job (W70: "the plan hash != the front's").
...
def _blocks_of(geom: ParamGeom, layout: GroupLayout, is_dst: bool) -> List[List[Block]]:
    ...
    if layout.tp_size == 1 and layout.n_ranks >= 1:
        ...
        if geom.stage is not None and not 0 <= int(geom.stage) < layout.n_ranks:
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree: {geom.name} names stage "
    ...
    if layout.tp_size != layout.n_ranks:
        raise Weg2XchgPlanDisagree(
            f"W70 Weg2XchgPlanDisagree: group {layout.name!r} has tp_size "
    ...
    if is_dst and geom.dst_widths is not None:
        if len(geom.dst_widths) != layout.n_ranks:
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree: {geom.name}: {len(geom.dst_widths)} "
...
def _live_storage(
    ...
    if live.itemsize != int(geom.itemsize):
        raise Weg2XchgPlanDisagree(
            f"W70 Weg2XchgPlanDisagree: {geom.name} on {layout.name!r} rank "
...
def _emit(
    ...
    for r, blocks in enumerate(src_blocks):
        ...
        if live is not None:
            ...
            if actual != derived:
                raise Weg2XchgPlanDisagree(
                    f"W70 Weg2XchgPlanDisagree: {geom.name} src_rank={r}: the "
...
def _check_tiles(
    ...
    for lo, hi in sorted(covered):
        if lo > cursor:
            raise Weg2XchgSourceMissing(
                f"W76 Weg2XchgSourceMissing: {geom.name} dst_rank={d_rank}: "
                f"units [{cursor}, {lo}) of this rank's {extent} have no VRAM "
                f"source and no ZEROFILL descriptor."
            )
        if lo < cursor:
            raise Weg2XchgPlanDisagree(
                f"W70 Weg2XchgPlanDisagree: {geom.name} dst_rank={d_rank}: unit "
    ...
    if cursor > extent:
        raise Weg2XchgPlanDisagree(
            f"W70 Weg2XchgPlanDisagree: {geom.name} dst_rank={d_rank}: the plan "
            f"writes {cursor} units into a destination whose own storage holds "
            f"{extent}. The shard vector and the hardware disagree."
        )
    if cursor < extent:
        raise Weg2XchgSourceMissing(
            f"W76 Weg2XchgSourceMissing: {geom.name} dst_rank={d_rank}: units "
...
def _check_cards(src: GroupLayout, dst: GroupLayout) -> None:
    ...
    if src.n_ranks != dst.n_ranks or any(
        ...
        raise Weg2XchgPlanDisagree(
            f"W70 Weg2XchgPlanDisagree: group {src.name!r} runs on cards "
...
def _check_bases(src: GroupLayout, dst: GroupLayout) -> None:
    ...
    if lo_a < hi_b and lo_b < hi_a:
        raise Weg2XchgPlanDisagree(
            f"W70 Weg2XchgPlanDisagree: group {src.name!r} occupies global "
...
def build_plan(
    ...
    for w, wave in enumerate(waves):
        for tag in wave:
            if tag in wave_of:
                raise Weg2XchgPlanDisagree(
                    f"W70 Weg2XchgPlanDisagree: tag {tag!r} appears in wave "
    ...
    for geom in sorted(inventory, key=lambda g: g.name):
        ...
        if geom.tag not in wave_of:
            if geom.tag not in skip:
                raise Weg2XchgSourceMissing(
                    f"W76 Weg2XchgSourceMissing: {geom.name} carries tag "
        ...
        if not descs:
            raise Weg2XchgSourceMissing(
                f"W76 Weg2XchgSourceMissing: {geom.name} (tag {geom.tag!r}) "
    ...
    if barren:
        raise Weg2XchgSourceMissing(
            f"W76 Weg2XchgSourceMissing: wave tags {barren} produced no "
            f"descriptor at all. A wave that moves nothing is an agreement "
            f"between six ranks that no byte has to arrive."
        )
    if not raw:
        raise Weg2XchgSourceMissing(
            "W76 Weg2XchgSourceMissing: the plan is empty. Its plan_id is a "
...
RUNNER_SHAPE_REFUSAL_MARKER = "W78 Weg2XchgRunnerShapeUnknown"


class Weg2XchgRunnerShapeUnknown(RuntimeError):
    """W78 -- a secondary ModelRunner this predicate cannot classify.
    ...
    guessing PRIMARY puts unsourced bytes into it (W76).

    W78 is free per the spec section 7 census (highest assigned W69, this
...
def runner_shape_refusal_message(shape: RunnerShape) -> str:
    return (
        ...
        f"VRAM source into it (W76). Classify the new producer in "
...
def roll_forward_weights_tag(*, has_draft_shard: bool) -> Optional[str]:
    """The tag W75's roll-forward reload may open its ONE region with.
    ...
    a destination range with no VRAM source (W76), while the draft tag does the
    ...
    the reload's shape and belongs to S6 with W75 (spec section 3.6).
    """
...
def roll_forward_refusal_message() -> str:
    return (
        ...
        "no VRAM source (W76). Refusing rather than re-tagging. OWNER: S6 -- "
        "W75's roll-forward needs one region per runner (spec section 3.6)."
    )


# ===========================================================================
# S2 -- COVERAGE ARMING (W69, spec section 2.4 rule 4 and section 6/S2)
# ===========================================================================

COVER_LINE_PREFIX = "WEG2-XCHG-COVER"
COVERAGE_REFUSAL_MARKER = "W69 Weg2XchgCoverageRefused"
...
def coverage_refusal_message(rows: Mapping[str, TagCoverage]) -> str:
    """The W69 text: every refused tensor named, with its module path."""
    parts: List[str] = []
...
def refuse_if_not_ok(vote: CoverageVote) -> CoverageVote:
    """Raise W69 for a failing vote.  ONLY where a group fence covers it."""
    if not vote.ok:
