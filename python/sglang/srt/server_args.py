...
class ServerArgs:
    ...
    draft_kv_layout: A[
        ...
        Arg(
            help=(
                ...
                "(MTP/NEXTN, --speculative-eagle-topk 1). "
                "WHICH SETTING IS RIGHT DEPENDS ON ONE THRESHOLD, and both "
                "sides of it are measured. "
                "TP > num_kv_heads (the kv heads cannot be split, so every "
                "rank already holds all of them): use 'dcp'. Here "
                "'replicated' is not merely less efficient, it is the "
                "DEGRADED configuration -- the draft is asked for a head "
                "shard that does not exist, and while the verified output "
                "stays correct its proposals are almost never accepted. "
                "Measured on Qwen3.5-2B, TP=3 over 2 kv-heads, vector "
                "[30,17,17]: 'replicated' spends 61 verify rounds on 64 "
                "tokens (accept 1.05 -- speculation paying its cost for "
                "nothing) against 20 rounds (accept 3.20) on 'dcp', with "
                "identical output ids; Qwen3.6-35B-A3B-FP8 the same shape, "
                "32 -> 20 rounds. 'dcp' additionally gives the full "
                "token-shard VRAM win there (draft KV 1.86 -> 0.62 GB, "
                "-67%%, exactly ratio_r/S per rank). "
                "TP <= num_kv_heads (heads split evenly): leave it "
                "'replicated'. The token-shard and head-replication factors "
                "CANCEL, so 'dcp' is VRAM-byte-neutral, and it costs "
                "speculation efficiency: measured on Qwen3.6-27B-FP8, TP=3 "
                "over 4 kv-heads -- output token ids byte-identical (64/64 on "
                "both probe prompts) but verify rounds 16->19 and 19->21, "
                "accept -10 to -16%%. That cost is benign LSE reassociation in "
                "the DRAFT (the sharded prefix is read as per-rank partials "
                "and recombined, flipping near-tie draft proposals), not a "
                "correctness defect."
            ),
    ...
    kv_session_offload_destinations: A[
        ...
        Arg(
            ...
            "pressure -- and at its exhaustion sites via "
            "park_instead_of_demote), the oldest "
    ...
    rank_moe_ratio: A[
        ...
        Arg(
            ...
            "uncalibrated, needs no prior boot, and measured 1.4307x on the "
            "transfer term / -6.42 %% end-to-end on the reference recipe "
            "(2026-08-03 green-corridor window; the night window reads "
            "1.4253x on the same basis). Transfer-term figures are quoted "
            "WORK-MATCHED -- read off the two arms' final dump revisions at a "
            "common work point, which is the only basis on which two arms may "
            "be divided by each other (#482/#523). "
            ...
            "overloaded the rank it thought was cheap and measured -0.94 %% "
            "end-to-end, i.e. inside the same-window A-vs-A floor of 4.09 %%. "
            "The rejection rests on that leg and on the mechanism above, NOT "
            "on the transfer term: work-matched, the calibrated arm reads "
            "1.4573x against plain 'link''s 1.4253x in that window and "
            "slightly WINS it -- the old '1.439x against 1.496x' sentence "
            "divided two counters sampled at different work points. "
    ...
    def _dump_observation_ledger(self) -> None:
        ...
        WHY THE BOOT THAT SHIPS HAD NO MODEL TO CHECK. The ledger is
        constructed only on the paths that USE it -- the gated reserve and the
        #593 full-demand reserve. The shipped config pins its budgets
        explicitly (``--rank-gpu-memory-mib 31800,14000,15600``), which is the
        pin path, and the pin path skips the planner. So on precisely the
        configuration that runs, no ledger was ever built, no
        ``ledger_<boot>.json`` was ever written, and ``reconcile.py`` -- the
        model-versus-measurement table that is the whole payout of #605 -- had
        nothing to compare the measured posts against. Fourteen boots of
        recorded marks sat beside zero modelled counterparts.

        This closes that loop and nothing else. The ledger built here is
        DISCARDED: it is dumped for the recorder and never returned, never
        consulted for a size, never allowed to move a byte of the boot it
        observes. A configuration that does not use the ledger keeps not using
        it; it merely stops being unobservable.

        Gated on the recorder already being armed, so an unarmed boot pays
        nothing, and wrapped whole: an observation must not be able to fail a
        launch. Failures land at debug level because a rig without NVML is a
        normal reason for this to be unavailable, not an incident.
        """
        import os
    ...
    def _handle_kv_session_offload(self):
        ...
        if self.enable_hierarchical_cache or self.enable_unified_memory:
            ...
            _kvso_bytes = int((self.kv_session_offload_host_ram_gib or 0) * (1024**3))
    ...
    def _pp_cut_kv_bytes_per_token_per_attn_layer(self):
        ...
        kv_heads = int(
            probe("num_key_value_heads") or probe("linear_num_key_heads") or 0
        )
