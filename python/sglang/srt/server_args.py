    ...
    draft_kv_layout: A[
        ...
        Arg(
            help=(
                ...
                "WHEN IT PAYS, measured: 'dcp' saves draft VRAM only where the "
                "kv-heads cannot be sharded further, i.e. TP > num_kv_heads -- "
                "there the saving is the full token-shard factor ratio_r/S. On "
                "a head-shardable rig the token-shard and the head-replication "
                "factors CANCEL and the layout is VRAM-byte-neutral while still "
                "costing speculation efficiency: measured on Qwen3.6-27B-FP8, "
                "TP=3 over 4 kv-heads, vector [30,17,17] -- output token ids "
                "byte-identical (64/64 on both probe prompts) but verify rounds "
                "16->19 and 19->21, i.e. accept length -10 to -16%%. That cost "
                "is benign LSE reassociation in the DRAFT (the sharded prefix "
                "is read as per-rank partials and recombined, flipping "
                "near-tie draft proposals), not a correctness defect -- the "
                "verified output is unchanged. Enable it for the VRAM win on "
                "TP > num_kv_heads deployments; leave it 'replicated' on rigs "
                "whose heads split evenly, where it costs accept and returns "
                "nothing."
            ),
