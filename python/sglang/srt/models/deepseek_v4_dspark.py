...
class DeepseekV4ForCausalLMDSpark(nn.Module):
    ...
    def _remap_mtp_rest(rest: str) -> str:
        ...
        which matches no parameter and is dropped with only a warning, so the
        draft "loads" with uninitialised scales and the speculative accept rate
        silently pins to zero. ``.scale_inv`` degrades the same way, into
        ``.weight_scale_inv_inv``. The target model's loader anchors this same
