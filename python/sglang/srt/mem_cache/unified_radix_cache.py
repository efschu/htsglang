...
class UnifiedRadixCache(KVCacheEventMixin, BasePrefixCache):
    ...
    def _attn_reduce_world(self) -> int:
        ...
        PRINTED IT AS ONE FOR THE WHOLE CAMPAIGN. `drain_retired_prefetch`
        (and, before #1066 deleted it, `take_agreed_reissue`) packs
        `[digest, -digest]` so that one MIN reduce
    ...
    def _weg2_publish_at_chunk(self, req, radix_key) -> None:
        ...
        try:
            ...
            if not first:
                return
            with _weg2_p_overlap.span("publish"):  # #PGAP
                stats = self.publish_unbacked_sweep(max_issue=_rp.max_issue(),
    ...
    def check_prefetch_progress(self, req_id: str) -> bool:
        ...
        if _stale:
            ...
            # #1066: the #943b owed-ledger entry that stood here is DELETED.
            # A stale refusal no longer creates per-round re-issue debt: the
            # cutover that made the stamp stale re-issues the whole affected
    ...
    def drain_storage_control_queues(self) -> bool:
        ...
        if _parts is not None:
            _parts["agree"] = _parts.get("agree", 0.0) + (time.perf_counter() - _ta)
