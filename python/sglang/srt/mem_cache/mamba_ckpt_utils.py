...
def lower_protected_to(req, new_protected_len: int) -> int:
    """Lower ``req.cache_protected_len`` to a length the tree really backs. #928.

    ONLY LOWERS, NEVER ASSIGNS, and that is the whole contract. ``cache_
    protected_len`` means "the tree owns the KV below this" -- the premise both
    free-sites in ``cache_finished_req`` rest on, and the one
    ``retention_shrinks_protected`` states above. Raising it would promise
    ownership nobody holds; assigning it flatly would clobber a HIGHER claim
    that a later, legitimate match had earned. So: ``min``.

    WHY IT IS NEEDED AT ALL. A refused resume (#928) zeroes the match and sends
    the request into a full re-prefill. Its old prefix rows are not its rows any
    more -- it will allocate new ones -- but ``cache_protected_len`` survived
    the zeroing, so the promise "the tree owns the KV below this" was still
    standing over rows the request no longer had. ``cache_finished_req`` then
    frees from that stale length and inserts only up to the retention, and the
    interval between them is owned by nobody: #935's measured per-request
    deficit, with the refusal as its producer.

    THE LOWERING BELONGS AT THE MOMENT OF ZEROING, not to the request as a
    whole. A request whose match is zeroed has no tree-owned prefix RIGHT THEN;
    a NEW successful match after the re-prefill may legitimately grant one
    again, and this must not stand in its way. That is why it is called from
    the refusal branches and nowhere else, and why it takes the length the
    caller just decided rather than reading one for itself.

    Returns the resulting length, so a caller or a test can assert on the value
    rather than on the absence of a symptom.
    """
    current = int(getattr(req, "cache_protected_len", 0) or 0)
    lowered = min(current, max(0, int(new_protected_len)))
    try:
        req.cache_protected_len = lowered
    except Exception:  # noqa: BLE001 - a bookkeeping fix may never break a match
        return current
    return lowered
