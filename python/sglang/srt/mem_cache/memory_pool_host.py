...
def _arena_id_tokens(pool) -> int:
    """H62 (NF line, x59): the arena id range of an arena host pool in TOKENS
    -- ``A * P`` on a paged pool, ``A`` when P == 1 (the 27B form). Imported
    at call time: the arena pool module imports from this one's neighbours."""
    from sglang.srt.mem_cache.pool_host.arena_pool import _atok

    return int(_atok(pool))
...
class HostPoolGroup:
    ...
    def _free_arena_rows(self, pool, indices):
        ...
        # H62 (NF line): the arena's id range counts TOKENS -- A * P on the
        # paged Next Flash pool (x59, P = 64), exactly A on the 27B's (P = 1).
        # The 27B bound S + A would read every id of a slot >= A / P as a
        # placeholder and drop it WITHOUT returning its reference.
        A = _arena_id_tokens(pool)
