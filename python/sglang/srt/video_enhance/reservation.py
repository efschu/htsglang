...
"""Cross-process VRAM reservation ledger, Class-3 side (DESIGN #333 §3.3).
...
This is not the full arbiter of §7 (that is M1). It is what a
process-isolated Class-3 tenant needs in order to declare and hold a static
budget honestly, plus the per-card serialisation primitive that profiling and
TensorRT engine builds require (§6.4: an engine build is an exclusive window
on that card, and it lasts minutes).
"""
...
class ReservationStore:
    ...
    def _resolve_total(self, card_uuid: str, override: int | None) -> int:
        ...
        from sglang.srt.video_enhance.nvml import total_bytes_for_uuid
