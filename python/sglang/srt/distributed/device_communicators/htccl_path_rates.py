...
"""Rate-table loaders for the HTCCL path dispatcher (#279).
...
"status quo, decide nothing" (htccl_path_dispatcher hard rule 1).
...
def load_rate_tables(
    ...
    for label, path, loader in sources:
        if path is None or not os.path.exists(path):
            logger.warning(
                "HTCCL path dispatcher: rate source %r not available (%s) -- "
    ...
    if gdr_tsv is None or not os.path.exists(gdr_tsv):
        logger.warning(
            "HTCCL path dispatcher: rate source '#278 gdr matrix' not "
    ...
    for err in merged.errors:
        logger.warning("HTCCL path dispatcher rate load: %s", err)
    return merged
