...
"""Size- and load-aware path dispatcher for HTCCL (#279) -- CPU skeleton.
...
(see ``htccl_path_rates``): the #278 GDR matrix, the pending NCCL/system-RAM
...
Wiring: ``SGLANG_HTCCL_PATH_DISPATCHER=1`` (default off) makes
``HTCCLCommunicator`` build a dispatcher and consult it in ``_select`` via
...
test/registered/unit/distributed/test_htccl_path_dispatcher.py.
"""
...
# transport_hint values the HTCCL _select hook can act on.
...
class PathProfile:
    ...
    # Actuation handle for the HTCCL _select hook (HINT_* above). Paths
...
class PathDispatcher:
    ...
    def register_path(
        ...
        if profile.provenance == PROVENANCE_PLACEHOLDER:
            logger.warning(
                "HTCCL path dispatcher: path %r registered with PLACEHOLDER "
    ...
    def round_boundary(self) -> None:
        ...
        with self._lock:
            if self._capture_active:
                logger.warning(
                    "HTCCL path dispatcher: round_boundary() during active "
...
# ---------------------------------------------------------------------------
# flag-gated construction + the thin HTCCL hook
# ---------------------------------------------------------------------------
...
def dispatcher_enabled() -> bool:
    ...
    return bool(int(os.environ.get("SGLANG_HTCCL_PATH_DISPATCHER", "0")))


def maybe_build_dispatcher() -> Optional[PathDispatcher]:
    """The HTCCLCommunicator entry point: None unless
    SGLANG_HTCCL_PATH_DISPATCHER=1. The freshly built dispatcher has an
    EMPTY registry, so every decision is status quo until rate tables are
    loaded (htccl_path_rates) -- flag on is byte-identical today."""
    if not dispatcher_enabled():
        return None
    logger.info(
        "HTCCL path dispatcher enabled (skeleton): empty registry, every "
...
def refine_transport_choice(
    ...
    """The thin #240 hook used by ``HTCCLCommunicator._select``.
    ...
    if hint == HINT_TRANSPORT and chosen is not None:
        return chosen
    logger.warning(
        "HTCCL path dispatcher: decision %r for op %r has no actionable "
