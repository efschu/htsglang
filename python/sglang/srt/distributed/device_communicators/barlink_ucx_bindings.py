...
def _assert_layout() -> None:
    ...
    for name, off in expect.items():
        ...
        if actual != off:
            raise RuntimeError(
                f"barlink_ucx_bindings: ucp_request_param_t.{name} laid out at "
    ...
    for cls, name, off in (
        ...
        if actual != off:
            raise RuntimeError(
                f"barlink_ucx_bindings: {cls.__name__}.{name} laid out at "
...
class UcpLibrary:
    ...
    ``SGLANG_BARLINK_UCX_LIB`` selects the library. When it names an explicit
    ...
    def __init__(self, path: str | None = None):
        self.path = path or os.environ.get("SGLANG_BARLINK_UCX_LIB", "libucp.so.0")
        ...
        try:
            ...
            raise UcxError(
                f"barlink ucx: cannot load '{self.path}' ({e}). Install UCX "
                f"(apt: libucx0) or set SGLANG_BARLINK_UCX_LIB to a libucp.so.0."
            ) from e
        try:
    ...
    def check(self, status: int, what: str) -> None:
        if status != UCS_OK:
            raise UcxError(f"barlink ucx: {what} failed: {self.status_string(status)}")
...
def _make_request_param(recv: bool = False) -> UcpRequestParam:
    ...
    ``MEMORY_TYPE = HOST`` is an assertion about barlink's design, not a guess:
...
class UcpWorker:
    ...
    def __init__(
        ...
        # SINGLE is the fast mode. barlink collectives are issued from one
    ...
    def post_send(self, peer: int, ptr: int, nbytes: int, tag: int):
        ...
        if req >= _ERR_PTR_FLOOR:
            raise UcxError(
                f"barlink ucx: tag_send(peer={peer}, tag={tag:#x}) failed: "
    ...
    def post_recv(self, ptr: int, nbytes: int, tag: int):
        ...
        if req >= _ERR_PTR_FLOOR:
            raise UcxError(
                f"barlink ucx: tag_recv(tag={tag:#x}) failed: "
                f"{self.lib.status_string(_ptr_status(req))}"
            )
        return req
    ...
    def _timeout(self, n_pending: int) -> None:
        raise UcxError(
            f"barlink ucx: {n_pending} request(s) still pending after "
    ...
    while remaining:
        for i, (progress, worker, check, free) in enumerate(handles):
            ...
            for req in reqs:
                ...
                if status != UCS_OK:
                    raise UcxError(
                        f"barlink ucx: request failed: {lib.status_string(status)}"
                    )
            pend[i] = still
            if not still:
                remaining -= 1
        ...
        if (
            ...
            raise UcxError(
                f"barlink ucx: {sum(len(p) for p in pend)} request(s) still "
                ...
                f"collective -- check that SGLANG_BARLINK_UCX_WORKERS is the "
