...
# vllm/distributed/device_communicators/barlink_device.py
"""barlink device-driven transport.
...
# test_barlink_port.py::test_import_does_not_initialize_cuda). By the time a
...
# The env overrides (SGLANG_BARLINK_RSAG_SHARES / SGLANG_BARLINK_PIPE_CHUNK_MIB)
...
// #583: DELIBERATELY UNUSED. Kept only so the hipify note above is not lost.
// Do NOT reintroduce a call to this from a spin loop: a device trap destroys
// the CUDA context, and every later CUDA call in the process -- including
// unrelated Triton module loads -- then fails with a sticky
// cudaErrorLaunchFailure that names the wrong kernel. Spin loops write the
// abort word in seq_dev[1] and return instead; the host raises from
// BarlinkDeviceTransport.check_aborted.
#if defined(__HIP_PLATFORM_AMD__) || defined(USE_ROCM)
#define BARLINK_TRAP() abort()
#else
#define BARLINK_TRAP() __trap()
#endif

using u64 = unsigned long long;

struct SlotPtrs {
  const void* p[BARLINK_MAX_RANKS];
};
...
__global__ void barlink_begin_kernel(u64* seq_dev,
  ...
  *seq_dev = seq;
  ...
  for (int r = 0; r < world; ++r) {
    while (vload(&cons_flags[r * 8]) < seq - 1) {
      // #583: write the abort word and RETURN -- never __trap(). A device
      // trap destroys the CUDA context, and every later CUDA call in this
      // process then returns a sticky cudaErrorLaunchFailure ("unspecified
      // launch failure") at whatever unrelated call site happens to be next,
      // which is how a barlink expiry used to surface as a Triton
      // load_binary error. The host reads seq_dev[1] in
      // BarlinkDeviceTransport.check_aborted and raises a structured
      // DeviceCollectiveAborted instead. Same contract as the BAR1
      // transport's ctlStatus word (#431 fix 2, barlink_bar1.py:4464).
      if (clock64() - start > timeout) { seq_dev[1] = 1ull; return; }
    }
  }
}

#define BARLINK_MAX_CHUNKS 64
...
// behind it.
__global__ void barlink_publish_kernel(volatile u64* pub_flags,
                                     const u64* seq_dev, int rank,
                                     int chunk) {
  __threadfence_system();
  pub_flags[rank * BARLINK_MAX_CHUNKS + chunk] = *seq_dev;
}

// K2: spin until every peer published chunk #c of op #seq.
__global__ void barlink_wait_kernel(const volatile u64* pub_flags,
                                  u64* seq_dev, int rank, int world,
                                  int chunk, u64 timeout) {
  u64 seq = *seq_dev;
  u64 start = clock64();
  for (int r = 0; r < world; ++r) {
    if (r == rank) continue;
    while (vload(&pub_flags[r * BARLINK_MAX_CHUNKS + chunk]) < seq) {
      // #583: abort word + return, never __trap(). See barlink_begin_kernel.
      if (clock64() - start > timeout) { seq_dev[1] = 2ull; return; }
    }
  }
  __threadfence_system();
}
...
__global__ void barlink_wait_one_kernel(const volatile u64* pub_flags,
                                      u64* seq_dev, int owner,
                                      int idx, u64 timeout) {
  u64 seq = *seq_dev;
  u64 start = clock64();
  while (vload(&pub_flags[owner * BARLINK_MAX_CHUNKS + idx]) < seq) {
    // #583: abort word + return, never __trap(). See barlink_begin_kernel.
    if (clock64() - start > timeout) { seq_dev[1] = 3ull; return; }
  }
  __threadfence_system();
}
...
template <typename T>
__global__ void barlink_add_kernel(const T* __restrict__ inp,
                                 T* __restrict__ out,
...
__global__ void barlink_done_kernel(volatile u64* cons_flags,
                                  const u64* seq_dev, int rank) {
...
void launch(const at::Tensor& inp, at::Tensor& out, at::Tensor& scratch,
  ...
  barlink_begin_kernel<<<1, 1, 0, stream>>>(seq, (const volatile u64*)cons,
                                          world, (u64)timeout);
  // D2H: DMA into the own mapped slot at full PCIe bandwidth.
  cudaMemcpyAsync((void*)slot_addrs[rank], inp.data_ptr(), nbytes,
                  cudaMemcpyDefault, stream);
  barlink_publish_kernel<<<1, 1, 0, stream>>>(pub, (const u64*)seq, rank, 0);
  barlink_wait_kernel<<<1, 1, 0, stream>>>((const volatile u64*)pub,
                                         (u64*)seq, rank, world, 0,
                                         (u64)timeout);
  ...
  barlink_add_kernel<T><<<blocks, threads, 0, stream>>>(
      (const T*)inp.data_ptr(), (T*)out.data_ptr(), (const T*)scr,
      world - 1, n);
  barlink_done_kernel<<<1, 1, 0, stream>>>(cons, (const u64*)seq, rank);
}
...
// graph-capturable.
static cudaEvent_t barlink_fork_ev = nullptr;
static cudaEvent_t barlink_join_ev = nullptr;
...
void launch_pipelined(const at::Tensor& inp, at::Tensor& out,
                      ...
                      cudaStream_t s2) {
  if (barlink_fork_ev == nullptr) {
    cudaEventCreateWithFlags(&barlink_fork_ev, cudaEventDisableTiming);
    cudaEventCreateWithFlags(&barlink_join_ev, cudaEventDisableTiming);
  }
  size_t n = inp.numel();
  int nchunks = (int)((n + chunk_elems - 1) / chunk_elems);
  TORCH_CHECK(nchunks <= BARLINK_MAX_CHUNKS, "barlink: too many chunks");
  u64* seq = (u64*)seq_dev.data_ptr();
  volatile u64* pub = (volatile u64*)pub_addr;
  volatile u64* cons = (volatile u64*)cons_addr;
  ...
  barlink_begin_kernel<<<1, 1, 0, s1>>>(seq, (const volatile u64*)cons, world,
                                      (u64)timeout);
  cudaEventRecord(barlink_fork_ev, s1);
  cudaStreamWaitEvent(s2, barlink_fork_ev, 0);
  ...
  for (int c = 0; c < nchunks; ++c) {
    ...
    barlink_publish_kernel<<<1, 1, 0, s1>>>(pub, (const u64*)seq, rank, c);
  }
  for (int c = 0; c < nchunks; ++c) {
    size_t off = (size_t)c * chunk_elems;
    size_t cn = std::min(chunk_elems, n - off);
    int blocks = (int)std::min<size_t>((cn + threads - 1) / threads, 128);
    barlink_wait_kernel<<<1, 1, 0, s2>>>((const volatile u64*)pub,
                                       (u64*)seq, rank, world, c,
                                       (u64)timeout);
    int p = 0;
    for (int r = 0; r < world; ++r) {
      if (r == rank) continue;
    ...
    barlink_add_kernel<T><<<blocks, threads, 0, s2>>>(
        in_ptr + off, out_ptr + off, (const T*)scr, world - 1, cn);
  }
  cudaEventRecord(barlink_join_ev, s2);
  cudaStreamWaitEvent(s1, barlink_join_ev, 0);
  barlink_done_kernel<<<1, 1, 0, s1>>>(cons, (const u64*)seq, rank);
}
...
void launch_rs_ag(const at::Tensor& inp, at::Tensor& out,
  ...
  size_t n = inp.numel();
  int nchunks = (int)((n + chunk_elems - 1) / chunk_elems);
  TORCH_CHECK(nchunks <= 32, "barlink rs_ag: too many chunks");
  ...
  for (int c = 0; c < nchunks; ++c) {
    ...
    cudaMemcpyAsync((void*)((T*)slot_addrs[rank] + r2off), out_ptr + off,
                    cn * sizeof(T), cudaMemcpyDefault, s2);
    barlink_publish_kernel<<<1, 1, 0, s2>>>(pub, (const u64*)seq, rank,
                                          32 + c);
  }
  ...
  for (int c = 0; c < nchunks; ++c) {
    ...
    size_t off = (size_t)c * chunk_elems;
    size_t cn = std::min(chunk_elems, n - off);
    barlink_wait_one_kernel<<<1, 1, 0, s2>>>((const volatile u64*)pub,
                                           (u64*)seq, owner, 32 + c,
                                           (u64)timeout);
...
void barlink_allgather(at::Tensor inp, at::Tensor out, int64_t rank,
                     int64_t world, at::Tensor seq_dev,
                     std::vector<int64_t> slot_addrs, int64_t pub_addr,
                     int64_t cons_addr, int64_t timeout) {
  TORCH_CHECK(world <= BARLINK_MAX_RANKS, "barlink: too many ranks");
  TORCH_CHECK(inp.is_contiguous() && out.is_contiguous());
  TORCH_CHECK(out.numel() == inp.numel() * world, "barlink: bad gather out");
  ...
  cudaMemcpyAsync((void*)slot_addrs[rank], inp.data_ptr(), nbytes,
                  cudaMemcpyDefault, stream);
  barlink_publish_kernel<<<1, 1, 0, stream>>>(pub, (const u64*)seq, (int)rank,
                                            0);
  barlink_wait_kernel<<<1, 1, 0, stream>>>((const volatile u64*)pub,
                                         (u64*)seq, (int)rank,
  ...
  barlink_done_kernel<<<1, 1, 0, stream>>>(cons, (const u64*)seq, (int)rank);
}

void barlink_allreduce(at::Tensor inp, at::Tensor out, at::Tensor scratch,
  ...
  TORCH_CHECK((int64_t)owner_weights.size() >= world,
              "barlink: owner_weights too short");
  TORCH_CHECK(inp.is_contiguous() && out.is_contiguous());
  auto stream = at::cuda::getCurrentCUDAStream().stream();
  ...
  switch (inp.scalar_type()) {
    ...
    case at::kFloat:
      ...
      break;
    default:
      TORCH_CHECK(false, "barlink: unsupported dtype");
  }
}
"""
...
def _local_arches(vendor: str) -> list[str]:
    ...
    if vendor == "hip":
        ...
        for i in range(torch.cuda.device_count()):
            ...
            if supported and name not in supported:
                logger.warning(
                    "barlink device transport: this torch build lists %s but the "
    ...
    if env and env.strip().lower() != "native":
        ...
        # other SGLANG_BARLINK* / build env in this transport).
...
_EXT_NAME_PREFIX = "barlink_device_ext"

#: Every barlink-owned cache entry. barlink_host.py and barlink_bar1_ext.py reuse
...
#: barlink_bar1_ext.py builds TWO extensions under two different names
#: (barlink_bar1_ext_<vendor>_<arches> for the kernels, barlink_bar1_dmabuf_ext
#: for the driver-ioctl exporter), so both prefixes are listed.
_EXT_NAME_PREFIXES = (
    _EXT_NAME_PREFIX,
    "barlink_host_ext",
    "barlink_bar1_ext",
    "barlink_bar1_dmabuf_ext",
)
...
def _ext_cache_guarded(name: str):
    ...
    try:
        ...
        logger.warning("barlink extension cache self-heal skipped: %r", exc)
        build_dir = None
        marker = None
...
def _load_ext(cpu_group):
    ...
    if _ext is None:
        ...
        name = "barlink_device_ext_" + vendor
        if arches:
        ...
        with _ext_cache_guarded(name) as build_dir:
            _ext = load_inline(
                ...
                functions=["barlink_allreduce", "barlink_allgather"],
                extra_cuda_cflags=flags or None,
                verbose=False,
                build_directory=str(build_dir) if build_dir is not None else None,
            )
        _info_once(
            "barlink device extension %r built for %s arch(es) %s in %.1f s "
            "(group: %s)",
...
class DeviceCollectiveAborted(RuntimeError):
    """A device-transport spin kernel hit its deadline and gave up.

    Carries the same structured context as the BAR1 transport's
    ``Bar1CollectiveAborted`` (#431 fix 2): which rank, which group, which
    kernel. Raised from :meth:`BarlinkDeviceTransport.check_aborted`.
    """
...
#: seq_dev[1] codes written by the spin kernels on deadline expiry.
_ABORT_KERNELS = {
    1: "barlink_begin_kernel (waiting for peers to consume the previous op)",
    2: "barlink_wait_kernel (waiting for a peer to publish its chunk)",
    3: "barlink_wait_one_kernel (waiting for the chunk owner in RS+AG phase 2)",
}


class BarlinkDeviceTransport:
    """GPU-driven all-reduce over the mapped shm segment.

    Reuses BarlinkShmTransport's segment/registration/rendezvous; only
    ...
    # ~30 s at 2 GHz. On expiry a kernel writes the abort word in seq_dev[1]
    # and RETURNS. It must never __trap(): #583 recorded a production boot in
    # which the trap destroyed the CUDA context, after which every unrelated
    ...
    def __init__(
        ...
        self._shm = BarlinkShmTransport(
            cpu_group=cpu_group, device=device, slot_bytes=slot_bytes
        )
        if not self._shm._pinned:
            raise RuntimeError(
                "barlink device transport needs a host-registered shm "
        ...
        # Header layout (see barlink_shm): CPU-path counters at offset 0;
        ...
        #
        # TWO elements, not one (#583). [0] is the sequence number; [1] is the
        # abort word a tripped spin kernel writes instead of trapping. Keeping
        # it in this tensor rather than a separate one is deliberate: every
        # kernel already receives `seq_dev`, so the abort path needed no new
        # parameter on any of the three launch templates or the two entry
        # points, and no captured graph changes shape.
        self._seq_dev = torch.zeros(2, dtype=torch.int64, device=device)
        #: Collectives launched since the last resolved read of the abort
        #: word. Zero means a check is free.
        self._unchecked_launches = 0
        #: True once a kernel of this transport has been recorded into a CUDA
        #: graph. A replay runs them with no host code in between, so this is
        #: what arms the replay-boundary check.
        ...
        self._pipe_chunk_bytes = (
            int(os.environ.get("SGLANG_BARLINK_PIPE_CHUNK_MIB", "4")) * 1024 * 1024
        )
        barlink_liveness.bounded_barrier(
            cpu_group,
            "barlink device bring-up barrier",
        ...
        # when SGLANG_BARLINK_PIPE_CHUNK_MIB or the tune bundle provides it).
        self._resolve_pipe_chunk()
        # #583: join the abort gate, exactly as the BAR1 transport does
        # (barlink_bar1.py:2224). This is what makes a tripped spin kernel
        # reach the host -- at the barlink dispatch sites and, for collectives
    ...
    def _resolve_owner_weights(self) -> list[int]:
        ...
        override = os.environ.get("SGLANG_BARLINK_RSAG_SHARES") or _tune_get(
            "rsag_shares"
        )
        if override:
            ...
            if len(w) != self.world_size or any(v <= 0 for v in w):
                raise ValueError(
                    f"SGLANG_BARLINK_RSAG_SHARES={override!r} must name "
        ...
        barlink_liveness.check_peers(
            "barlink device RS+AG link calibration (all_gather_object)",
        ...
        w = [v // g for v in w]
        _info_once(
            "barlink RS+AG link calibration: %s GB/s -> ownership weights "
            "%s (skip with SGLANG_BARLINK_RSAG_SHARES=%s)",
    ...
    def _resolve_pipe_chunk(self) -> None:
        ...
        if "SGLANG_BARLINK_PIPE_CHUNK_MIB" in os.environ:
        ...
        for mib in candidates:
            ...
            barlink_liveness.bounded_barrier(
                self.cpu_group,
                f"barlink pipe-chunk sweep pre-warmup barrier ({mib} MiB)",
                table=_peer_table_of(self),
            )
            ...
            barlink_liveness.bounded_barrier(
                self.cpu_group,
                f"barlink pipe-chunk sweep pre-timing barrier ({mib} MiB)",
                table=_peer_table_of(self),
            )
            t0 = time.perf_counter()
            for _ in range(5):
        ...
        barlink_liveness.check_peers(
            "barlink pipe-chunk sweep result exchange (all_gather_object)",
            table=_peer_table_of(self),
        )
        ...
        if self.rank == 0:
            logger.info(
                "barlink pipeline-chunk calibration: %s -> %d MiB (skip with "
                "SGLANG_BARLINK_PIPE_CHUNK_MIB=%d)",
                ", ".join(
    ...
    def barlink_broadcast(self, comm, tensor: torch.Tensor, src: int):
        ...
        alongside barlink, because the speculative path preferred pynccl for its
        capture-time syncs. Suppressing PyNccl under barlink (required: NCCL
    ...
    def all_gather(self, input_: torch.Tensor, dim: int = -1) -> torch.Tensor:
        ...
        assert nbytes <= self.slot_bytes, (
            f"barlink device all_gather: payload {nbytes} B exceeds the shm "
            "slot; raise SGLANG_BARLINK_SLOT_MIB."
        )
        input_size = inp.size()
        output = torch.empty(
    ...
    def _note_launch(self) -> None:
        """Record one launched collective for the abort check (#583)."""
        from sglang.srt.distributed.device_communicators.barlink import (
            graph_capture_running,
        )

        if graph_capture_running():
            # A replay runs the kernels with no host code in between, so the
            # per-collective counter cannot see it. This flag is what arms the
            # replay-boundary check instead.
    ...
    def check_aborted(self, where: str) -> None:
        ...
        if self._seq_dev is None or graph_capture_running():
            return
        ...
        raise DeviceCollectiveAborted(
            ...
            f"{self.rank}/{self.world_size} gave up in {kernel} after "
            f"{self._TIMEOUT_CYCLES} device cycles (~30 s at 2 GHz). The "
            f"output buffers of that collective are partially written, so "
            f"the run cannot continue. A peer either died, diverged from the "
            f"collective sequence, or was starved of PCIe bandwidth long "
            f"enough to miss the deadline -- the barlink device path and the "
            f"HiCache write-through tier share the same copy engines. "
            f"{barlink_abort_gate.ENV_ENABLE}=0 silences this check and "
            f"restores the pre-#583 behaviour of continuing over corrupt "
            f"buffers; it does not make the collective succeed.",
            rank=self.rank,
    ...
    def close(self) -> None:
        """Release the underlying shm segment (see BarlinkCommunicator.close)."""
