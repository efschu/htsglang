"""Bounded waits with peer liveness for the HTCCL collective family.
...
2. DEVICE side. The BAR1 spin kernels do carry a cycle deadline
   (``SGLANG_HTCCL_BAR1_CAP_CYCLES``, ~30 s at 2 GHz), but it is
...
* The liveness probe is rate-limited to ``SGLANG_HTCCL_PEER_PROBE_S``
...
* ``PeerWatchdog`` is one daemon thread per process. It sleeps
  ``SGLANG_HTCCL_PEER_PROBE_S`` between probes and touches no CUDA state on
  the healthy path.
...
DEFAULT PATH
------------
With ``SGLANG_HTCCL_PEER_LIVENESS=0`` every helper degrades to the exact
blocking call it replaced, and the abort window is never allocated, so the
kernels see a null pointer and skip the probe. With no HTCCL transport in
...
ENV_ENABLE = "SGLANG_HTCCL_PEER_LIVENESS"
...
ENV_TIMEOUT_S = "SGLANG_HTCCL_PEER_TIMEOUT_S"
#: How often a stalled wait may ask whether the peers still exist.
ENV_PROBE_S = "SGLANG_HTCCL_PEER_PROBE_S"
...
ENV_WATCHDOG = "SGLANG_HTCCL_PEER_WATCHDOG"
...
def register_peer_table(table: PeerTable) -> None:
    ...
    logger.info("HTCCL peer liveness: %s", table.describe())
...
class AbortWindow:
    ...
    def _build(self) -> None:
        try:
            ...
            logger.warning(
                "HTCCL peer liveness: no device-mapped abort word (%s). The "
    ...
    def trip(self, reason: str) -> None:
        ...
        if self._buf is not None:
            self._buf[0] = 1
        logger.error(
            "HTCCL peer liveness: abort window tripped -- %s. Spinning "
...
class PeerWatchdog:
    ...
    def start(self) -> PeerWatchdog:
        ...
        self._thread = threading.Thread(
            target=self._run, name="htccl-peer-watchdog", daemon=True
        )
        self._thread.start()
        return self
    ...
    def _run(self) -> None:
        while not self._stop.is_set():
            try:
                ...
                logger.exception("HTCCL peer watchdog probe failed")
...
def install(cpu_group: Any) -> Optional[PeerTable]:
    ...
    try:
        ...
        logger.warning(
            "HTCCL peer liveness: identity exchange failed (%s); bounded "
...
def _dead_peer_error(label: str, dead: Sequence[PeerIdentity], waited: float) -> str:
    ...
    return (
        f"HTCCL collective '{label}' cannot complete: {who} no longer "
...
def _timeout_error(label: str, timeout_s: float, table: Optional[PeerTable]) -> str:
    ...
    return (
        f"HTCCL collective '{label}' made no progress for {timeout_s:g}s and "
