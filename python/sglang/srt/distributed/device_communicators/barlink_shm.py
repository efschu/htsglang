...
# vllm/distributed/device_communicators/barlink_shm.py
"""barlink shared-memory transport.
...
def _pin_host_memory(ptr: int, nbytes: int, device: torch.device) -> bool:
    ...
    try:
        ...
        if ret != 0:
            logger.warning(
                "barlink: host-register of the shm segment failed (rc=%d); "
                "falling back to unpinned copies (slower).",
                ret,
            )
        return ret == 0
    except OSError as e:
        logger.warning("barlink: runtime library not loadable (%s); unpinned.", e)
        return False


class BarlinkShmTransport:
    ...
    def __init__(
        ...
        barlink_liveness.check_peers(
            "barlink shm rendezvous (broadcast_object_list)",
        ...
        barlink_liveness.bounded_barrier(
            cpu_group,
            "barlink shm bring-up barrier",
            table=self._peer_table,
        )
        _info_once(
            "barlink shm transport up: %d ranks x %d MiB slots, pinned=%s",
    ...
    def _wait_all(self, phase: int, seq: int | None = None) -> None:
        ...
        shared module policy (``SGLANG_BARLINK_PEER_TIMEOUT_S``, scaled while
    ...
    # BarlinkCommunicator.all_reduce. all_gather/reduce_scatter were never
    # served by shm and still are not; making them shm-capable is a separate,
    # testable change (barlink.py notes it as the one op-coverage gap).
    ...
    def barlink_all_reduce(self, comm, inp: torch.Tensor) -> torch.Tensor:
        ...
        dispatched as out-of-place. See BarlinkCommunicator._get_out_buf.
        """
    ...
    def all_reduce_(self, flat: torch.Tensor) -> None:
        ...
        assert nbytes <= self.slot_bytes, (
            f"barlink shm slot too small: {nbytes} > {self.slot_bytes}"
        )
