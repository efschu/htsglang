...
class DecodePreallocQueue(DecodeHiCachePreallocMixin):
    ...
    def _init_kv_manager(self) -> CommonKVManager:
        ...
        # draft pool is appended below. This is what the decode arm announces
        # to the prefill arm so the prefill can split its peer's flat pointer
        # list on the real section boundary instead of halving it.
        ...
        if self.enable_staging and not self.is_mla_backend:
            ...
            if per_rank_kv_heads > 0:
                ...
                # Use model config total, NOT per_rank * tp_size: the latter is
                # wrong under uneven TP when ranks hold different head counts.
                # Matches prefill.py:201-202 (model_config.get_total_num_kv_heads).
