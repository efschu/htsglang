...
def create_app(
    ...
    async def proxy(request: web.Request) -> web.StreamResponse:
        ...
        to_local = model is not None and model in request.app[LOCAL_MODELS]
        ...
        if to_local:
            ...
            if request.app[APPLY_SHIM] and request.path == "/v1/messages":
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info(
        "listening on %s:%d, local models %s -> %s, everything else -> %s",
        args.host,
        args.port,
