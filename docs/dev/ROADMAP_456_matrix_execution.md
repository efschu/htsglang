...
| ~~#439 confirmation band~~ **RAN 2026-08-03** | #439 | — | CONFIRMED: `compute` **1.4253x** on the transfer term work-matched (predicted 1.411x; the 1.496x this row carried is the pre-teardown revision and is withdrawn, #482/#523), -7.67 % end-to-end above a 4.09 % same-window floor; `compute-cal` FALSIFIED — on its END-TO-END leg (-0.94 %, inside the floor) and its mechanism, not on the transfer term, where work-matched it reads 1.4573x and slightly wins. Evidence `2026-08-03_439_confirm/RESULTS.md`. Two follow-ups: `--rank-auto-reserve-mib auto` infeasible on this recipe, and every arm CORRIDOR-RED (3080s at 211-251 MiB vs the 400 MiB floor) |
...
| 3 | #306 lossless cold-tier compression — desk falsification | #306 | none — sample-based, no card (`ANALYSE_456` §2.2 cell 2) | **DONE 2026-08-03 — `ANALYSE_306_lossless_ratio.md`. Verdict: DEAD for the expert asset, do not build a codec.** 112 real samples x 16 MiB across 14 asset classes, 2 064 verified-lossless round trips. DSV4-Flash routed experts (IQ2_XS/IQ3_XXS/IQ3_S/MXFP4 — *not* Q3_K, see §4.1) reach 1.026-1.091 against a 1.59-1.72 serial break-even; H0 = 7.58-7.95 bits/byte and `zstd --ultra -22 --long` finds 1.000-1.051, so the payload is at its entropy ceiling. ALIVE only under a pipelined mover, and only for FP8 (1.211x) / INT8 (1.131x) weight bytes on links <= 2.83 GB/s. Disk-image cell closed by a different mechanism: `/spinning` ZFS transparent compression already delivers 1.1735x on the #89 image (app-level zstd adds 1.9 %), and 12.64 % of that image is zero pages — **1.145x is available from a sparse write with no decompress stage**, which is the one piece worth taking. fp8-KV and GDN blobs are **SAMPLE-ABSENT** (no such dump exists on this box; needs a serving-boot dump, §4.2). nvcomp-class GPU decompress: **ABSENT**, never measured here, not invented. |
...
| item | task # | prerequisite / gate | evidence expected |
|---|---|---|---|
| #126 cold-expert Q2-class quantisation | #126 | **(a)** every lossless gain above landed (WAVE 0-4, in particular #306 — **the #306 half of (a) is discharged as of 2026-08-03: there is no lossless gain to land on the expert asset**, `ANALYSE_306_lossless_ratio.md` §8; the remaining items of (a) are untouched); **(b)** a quality gate on cold-only-expert accuracy loss, comparable in rigor to the #77/#120 bar already applied to Marlin expert-offload | bytes-per-miss reduction figure AND a quality delta, reported together — a bytes number with no quality number next to it does not clear this gate. #306's measurement adds one constraint on the arithmetic: the expert payload is already at its entropy ceiling, so #126's bytes-per-miss must be computed from the target bit width directly and **must not be stacked on any lossless factor** |
