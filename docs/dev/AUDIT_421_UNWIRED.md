...
| F6 | #407 memtier registry | INERT — **FIXED**, see §7 | `srt/memtier/` (registry, tiers, probe, profile, reservations): zero production importers and zero production symbol references outside its own package. `SGLANG_MEMTIER_PROFILE` is read at `memtier/profile.py:361`, on a path a serving process never executes. Found independently by module reachability and by the env triage. | Medium | L |
...
| §3 "memtier registry […] All new spill/offload consumers must pick targets from it" | Was stated as an active constraint while it was aspirational. #410 made it true for NEW consumers (one, `session_checkpoint.py`); the two offload consumers audited here (#286 register, #394 cold tier) predate the rule and still pick targets without it (memtier cuts 4/5). | Catalog overstated; now scoped to "new" |
...
| F6 | Medium | **FIXED (boot-pending)** — #410 session checkpoints resolve their tier through `memtier/consumers.py`; the inverted pin is retired and replaced by a positive call-site pin. The PRE-EXISTING consumers (#286 register, #394 cold tier) still carry their own target lists — memtier cuts 4/5 — so the catalog rule is honoured by one consumer, not enforced across all of them | `managers/session_checkpoint.py`, `memtier/consumers.py` |
