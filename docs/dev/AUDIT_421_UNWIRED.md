...
| F6 | #407 memtier registry | **FIXED (#286)** — was INERT | `srt/memtier/` (registry, tiers, probe, profile, reservations) had zero production importers and zero production symbol references outside its own package. `SGLANG_MEMTIER_PROFILE` is read at `memtier/profile.py:361`, on a path a serving process never executes. Found independently by module reachability and by the env triage. | Medium | L |
...
| §3 "memtier registry […] All new spill/offload consumers must pick targets from it" | Was stated as an active constraint while being aspirational: zero production consumers (F6). **Partly resolved by #286**: `model_executor/short_term_offload_register.price_park_target` is the first production consumer, for the classes it owns. The other consumers (expert offload, #394 cold tier, the rest of the #286 park-target ladder) still carry their own target lists, so the rule is now *partly* enforced, not enforced. §3 says so. | Catalog overstated; corrected |
...
| F6 | Medium | **FIXED (#286)** — first production consumer wired | pin retired, replaced by the positive `MemTierIsNowWiredTest` |
