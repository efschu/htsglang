...
| | |
|---|---|
| **Deep root-cause search in this repository** | **not usable.** 8 of 9 attempted bug runs consumed the entire 600 s budget without producing any answer. |
...
| b525 | test honesty | exact — **but contaminated** | exact | yes | 184 | 975 047 | — |
| **b527** | test honesty | miss (by file) | **exact** | no | **209** | **224 364** | — |

(b443, b446, b501 and the sanitized b525r were still queued behind the
translator tenant when this was written; see §7.)
...
* **Open-ended root-cause search in this repository.** Evidence: 8 of 9 bug
  runs (b289, b300, b383, b392, b406 and the earlier b406 attempt) consumed
  the full 600 s and returned the wrapper's "Execution error" — no answer at
  all. This is the primary finding and it is not marginal: it is 0 % delivery
  on the class, at 1.8-3.9 M tokens per failed attempt.
...
* **The b525 contamination was found, not designed out.** It is the only
  ...
  that read history and did not say so.
* **Three bugs (b443, b446, b501) and the sanitized b525r had not completed**
  when this was written — they were queued behind the translator tenant. The
  tier list above rests on the classes that did run; the silent-wrongness class
  is represented in Arm A by nothing yet, which is a real gap in the stratified
  design as executed.
