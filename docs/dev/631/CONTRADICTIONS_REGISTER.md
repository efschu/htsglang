...
| C17 | the corridor law is enforced | `CorridorGuard.ensure_headroom` implements item 15a's spill-before-alloc | **true but partial**: it is called from ONE site, the flip seam. A 272k bs1 prefill walks the binding card to 1001 MiB and only the next seam pulls it back. The law is unenforced at prefill admission | HANDOFF_677 §1a-bis |
