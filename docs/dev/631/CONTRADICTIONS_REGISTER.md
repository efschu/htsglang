...
| C22-b | what the `PHASE-FLIP payload checksum mismatch` actually reports, and what causes it | MERGE-R7 and the #656 acceptance both read it as C22's #657 corridor steering re-sorting on a rank-local 1 s clock, i.e. a DATA divergence the checksum caught | **the attribution is falsified and the message is a misreport.** The acceptance boot ran `corridor_post_sizing_mib=None` and `SGLANG_CORRIDOR_REBALANCE=0`, so the steering path was NOT enabled, and it failed anyway at 13:03:16Z after 320 clean cutovers. The log carries its own falsifier: a uint8 sum over N bytes lies in `[0, 255N]`, and the two `sender` fields were `4626949667419791296` (needs an 18-petabyte payload) and `-4450328002521349435` (**negative**). Neither was ever a checksum, so the DATA was never what differed. **Root cause: the wire FRAME is never agreed.** The per-peer payload length is a product of rank-local terms -- the live slot set (`build_flip_live_slots_fn` asserts "Replicated" and nothing verifies it) and the wave partition (whose own docstring names `_pools_alias` and `SGLANG_FLIP_SEAM_WAVES` as rank-local) -- nothing on the wire carries a length, and the receiver's size check compares `payload.numel()` against `incoming_nbytes[peer]`, **a buffer it allocated itself at that size**, so it is vacuous by construction. When the frames diverge NCCL delivers the shorter count, completes, and the trailer is read out of the unwritten tail of a `torch.empty` buffer. The hermetic suite could not see it because `_MailboxExchange` hands the receiver the SENDER's tensor. This UNIFIES the two incidents rather than replacing one with the other: the #657 steering re-sorts the free list on a private clock, which changes which rows a rank holds, which changes its frame -- so the steering was a TRIGGER, and disabling it removed that trigger and not the defect. Fixed by a pre-move frame ballot riding the ballot the round already runs (`_frame_digest`, reduced as the `[x, -x]` MIN pair), which ABANDONS unanimously instead of raising at the seam, plus `checksum_is_representable()` so the two diagnoses stop being conflated. `test_flip_frame_agreement_656.py`, 10 passed, red arm reproduces the metal signature on a CPU desk. **CONFIRMED ON METAL 2026-08-13 14:46:39Z, and the diverging quantity is now a NUMBER:** the ballot fired during a cold 280012-token prefill -- PP0/PP2 framed digest 1545804850, PP1 framed 1237458399, all three abandoned unanimously, no KvReshardError, no SIGQUIT, the process lived. PP1 is the same rank both peers blamed in the acceptance. The pool census at that instant: PP0/PP2 `free=309574 unaccounted=0`, **PP1 `free=269170 unaccounted=40404`** -- exactly the difference. All three agree on the resident request and on `cached=268816`, so it is NOT the request: rank 1's allocator holds 40404 rows no tree entry and no resident request accounts for, and 40404 rows x row_bytes IS the payload-length mismatch. **BUT the abandon is not benign:** a divergence that persists starves the pp->tp leg, the seam cap guard declares it unfundable, and the instance WEDGES (alive, KV intact, /health 503, no tokens) -- the 411-abandon deadlock class by another route. Strictly better than the SIGQUIT it replaces, and not 'serving continues'. The fix is AGREEMENT (broadcast or intersect the live slot set), not detection | ACCEPTANCE_656 §2c -> HANDOFF_REMEDIATION_656 §1 |
| C22-c | WHY rank 1's allocator holds 40404 rows its peers do not enumerate (HANDOFF_REMEDIATION_656 §7 left this as "the next question") | open; the suspect families were rank-divergent prefix-cache evolution (#616B), kvso/spill sentinel rows, radix eviction divergence, and the pressure-ladder rungs | **none of those. The shrink is collective and the RECOVERY was not, and R1's own log named it twice.** Comparing all 1980 POOL CENSUS lines of `boot_m1.log` rank by rank gives 7 divergent events in exactly two episodes, both opening at a `tp_to_pp` **post-cutover** -- the moment the KV backing is recovered. Two lines in the whole 33-minute boot say what ran there, both on PP1, both at those instants: `KV-BACKING recovered to 544077 of 578390 rows (corridor-bounded)` and `... to 537986 of 578390 (corridor-bounded)`. `578390-544077 = 34313` and `578390-537986 = 40404` -- **both census deltas, to the row**, and no other rank logged a corridor-bounded recovery all boot. `collective_kv_target` decides the SHRINK once for the group (which is why every `KV-BACKING released` line is identical on all three ranks); `KvBackingRelief.recover` is bounded by `self._free_bytes() - law`, **this rank's own** distance from the corridor law, so the binding card recovers less and stays capped where its peers do not. A capped rank enumerates a different id space, the seam's payload length is derived from that enumeration, and the frames diverge -- persistently, because a cap does not clear itself. The four suspects are excluded by the same census: all three ranks report `cached=268816` identically at every divergent event (so not the tree, not eviction), kvso is refused outright under `pp_size>1`, and the ladder's rungs return DRIVER pages and touch no allocator row. Closed by `cap_proposal`/`collective_cap_target`/`reconcile_to`, riding the KV rung's existing reduction widened 4 -> 8 fields. **METAL 2026-08-13 16:00:30Z: three ranks at 579870/579722/578606 exposed rows levelled to 578606 in ONE round, and every census field was identical from the next round on** | HANDOFF_REMEDIATION_656 §7 -> HANDOFF_REMEDIATION_656_R2 §1 |
| C22-d | what "the group agrees on the pool" has to include | the exposed ROW COUNT: level the ranks and their live slot sets must match | **the ORDER too, and the pool census cannot see it.** boot_v2, 16:00:42Z: the cap agreement had just levelled the group and all three POOL CENSUS lines were identical in every field (`size=579870 free=278572 cached=300034 unaccounted=1264`) -- and the frames diverged anyway, six rounds running (PP1 250257408 against 1658515222). `KvRowCap.release` restores withheld ids with `torch.sort`, deliberately ("the allocator takes from the FRONT"), while `_apply` is a mask filter that PRESERVES eviction order. So the rank that had to change its cap ended SORTED and the rank that did not kept eviction order, **with identical membership** -- and the allocator takes from the front, so they hand the next request different row ids. Divergent live slot sets with nothing in the census to show for it. Two corrections, both making the decision group-symmetric: the "nothing to do" verdict moves out of `reconcile_to` and into `collective_cap_target`, where it is taken from the REDUCED view (a rank may not decide for itself that it can skip a group operation), and `KvRowCap.sort_free_lists` makes the order a function of MEMBERSHIP alone rather than of which branch happened to run. Also why the ballot now digests in three parts (slots/waves/geometry), each riding the same reduction: a divergence the census cannot explain must still name its term | HANDOFF_REMEDIATION_656_R2 §4 |
| C22-e | what a phase-flip controller does when it cannot flip | HANDOFF_REMEDIATION_656 §1: the ballot converts a crash into a WEDGE -- alive, KV intact, /health 503, no tokens -- "strictly better than the SIGQUIT it replaces" and left as the top open item | **a wedge is not an acceptable terminal state and it did not need the divergence fixed first.** Strict purity is a THROUGHPUT rule, never a correctness one -- its own `threshold:<n>` and `off` modes run decode in the PP layout as supported configurations, and the documented cost is latency and throughput. So when the layout a work class needs is unreachable, the prohibition on THAT class is lifted, keyed on the stuck DIRECTION, driven by two already-reduced quantities, logged once, and self-clearing on the next committed flip. **METAL 2026-08-13 16:01:24Z: a persistent divergence blocked tp_to_pp, all three ranks relaxed purity for PREFILL simultaneously, and the instance kept serving -- /health 200, tokens flowing, load driver advancing**, where the same state was 503-with-no-tokens on R1's boot. It also unblocked axis 3 (C21-c): the gate had long measured its worst in-cutover draw and refused to act on the prediction *because* "refusing pp->tp starves decode outright", and that premise is now false | HANDOFF_REMEDIATION_656 §1 -> HANDOFF_REMEDIATION_656_R2 §2 |
| C21-c | where the remediation boot's residual 12 MiB corridor breach is made | HANDOFF_REMEDIATION_656 §5: a bs=1 deep-prefill transient -- "138 MiB at a seam became 12 MiB at a prefill", a different and much smaller mechanism | **falsified by the 100 ms trace it was read from.** Both sub-law samples sit at `14:42:25.634` and `.734`, and the seam census for that instant reads `CORRIDOR LAW BROKEN during tp_to_pp rank 1 at stage 'weights_refill': free 1012 MiB`, three seconds after `seam entry margin YIELDED (tp_to_pp) after 2 consecutive abandoned attempts`. The stage walk shows it directly: `backing_restore free=1250 | kv_write 1250 | gdn_state 1250 | weights_refill free=1012 step-238 | cutover free=1290`. It is the acceptance's OWN mechanism -- entry on the corridor law alone after a C20 yield, then the in-cutover draw -- 12 MiB deep instead of 138. The deep-prefill phase is WHEN, not WHY. Closed by withholding the YIELD when this rank's own measured draw predicts a sub-law trough, which only became available once the wedge stopped being the cost of a refusal (C22-e). The objection carries the margin-delay tag, exempt from the seam-abandon cap, so a self-clearing condition cannot stand the flip down for good | HANDOFF_REMEDIATION_656 §5 -> HANDOFF_REMEDIATION_656_R2 §3 |
| C-B1 | how far the "intermittent empty completion on deep prefill" (defect B) reaches | ACCEPTANCE_656 and HANDOFF_REMEDIATION_656 §4 both counted the load driver's rungs as content ("tokens flowed"), and built the defect's shape from the yarn probes on top of them | **most of the evidence base was not the defect.** Counted across both prior boots, EVERY load-driver prompt above 21 tokens returned a one-token completion -- 4001 (55x / 107x), 16001 (14x / 27x), 32001 (15x / 28x), 64001 (15x / 28x) -- 100%, at every depth, on both boots. Far too uniform for an intermittent defect, and the difference is one line: the rungs are pure filler and **ask nothing**, while the yarn probes append a secret and a question. Control, boot_v2 15:56Z, same filler and count with one question added: `in=4001 completion=1` versus `in=4026 completion=12 cached=0 text='...BANANA47'` -- the correct answer from a needle 4000 tokens back on a cold prefill. **The one-token completions are the model emitting EOS on a prompt that asks nothing**, not a serving defect. What remains of defect B is only the probes that DO ask: 280026 has now returned empty on 3 of 4 cold attempts (acceptance run 1, R1's load phase 2, boot_v2 15:57Z) while 250026/300026/338916 are exact -- which a ceiling cannot explain, since a ceiling cannot be passed by going deeper | ACCEPTANCE_656 axis 1 -> HANDOFF_REMEDIATION_656_R2 §5 |
| C21-b | where the #656 corridor breach is made, and what to spend to close it | ACCEPTANCE_656 open ticket 2: "give rank0 roughly 200-300 MiB back in the budget vector" | **wrong rank and wrong quantity.** By register C21's own card rule (`--rank-gpu-id` is CUDA order, FASTEST_FIRST) **rank0 is the 5090** -- it never came near the law (minimum 1941 MiB) -- and the breaching smi `gpu0` column is **rank 1**. And there is no steady-state shortfall to refund: gpu0's median free is 1788 MiB and its p1 is 1220. All 5 breaches begin 2-4 s after a C20 seam-entry-margin YIELD **on that same rank**: PP1 refused the entry margin twice and the yield policy then entered on the corridor LAW alone, which the yield's own line says leaves it "exposed to the in-cutover draw". Entries land at 2738-2804 MiB where law+drawdown needs 2876 -- a worst-case shortfall of **138 MiB**, exactly the breach depth. The drawdown itself measures 1814-1852 MiB (upper bound: a 9 s window, so concurrent prefill is inside it) against a gate that arms at law+512. Closed as a SIZING term, not a gate term: the seam solver's margin 192 -> 384 MiB (192 error bar + 138 measured shortfall + 38 measured spread), pool re-derived 597106 -> 578390 tokens. The law and the arming floor are now ONE declared pair (`corridor_guard.arming_floor_mib`, `check_threshold_pair`) and the runtime AUDITS its own corridor trace instead of only arming it | ACCEPTANCE_656 §2b -> HANDOFF_REMEDIATION_656 §2 |
...
| C605-1 | why the sizing path leaves the card above the corridor | "it reads torch's `reserved`, which is inflated by up to 7162 MiB of unbacked reservation" -- plausible and the right ORDER of magnitude | **falsified on the CUDA path**: `get_available_gpu_memory` (`utils/common.py:810`) reads `torch.cuda.mem_get_info(gpu_id)` -- the DRIVER's free -- after an `empty_device_cache`, and never consults `reserved`. The unbacked reservation is real and measured (4584-8344 MiB/rank) but is not the cause by this route | HANDOFF_605_FLIGHTRECORDER §4 |
| C605-2 | what `non_torch_bytes` reports on the ship config | the non-torch residue: CUDA context + NCCL + JIT + driver windows | **it reported a FALSE ZERO.** Written as `max(0, nvml_self - reserved)`; on this build torch reports up to 7162 MiB MORE reserved than resident, so the subtraction went negative and the field read 0 for a rank whose context measured 886 MiB. A floor built for sub-MiB quantisation was swallowing gigabytes. Split into `non_torch_measurable` + `unbacked_reservation_bytes` in `eb5ace7dff` | HANDOFF_605_FLIGHTRECORDER §1b |
| C605-3 | which rank a flight mark belongs to | the mark's `rank` field / the `flight_marks_rank{n}.jsonl` file it sits in | **neither, under PP.** Marks are filed by TP rank and the ship config is `--tp-size 1 --pp-size 3`, so the TP rank is 0 in ALL THREE processes and all three append to rank 0's file. Grouping by rank merged three cards into one timeline and billed a 3080's 482 MiB context to the 5090. Group by **pid** | HANDOFF_605_FLIGHTRECORDER §1a |
| C605-4 | what torch's `reserved` counter means on this build | device memory the process holds | **the KV pool's LOGICAL size, not its physical footprint.** The pool is a CUDA VMM arena (`mem_cache/kv_vmm_backing.py`, `256 GiB virtual; free until committed`) exposed as a `torch.cuda.MemPool` via `CUDAPluggableAllocator`, committed incrementally by `commit_range`/`decommit_range` against a watermark. `reserved - resident` IS the uncommitted span. Never calibrate a ledger term from `reserved` here | HANDOFF_605_FLIGHTRECORDER §4a |
| C605-5 | the KV pool is sized differently on every boot of one config (HANDOFF_605_FLIGHTRECORDER §3: `kv_pool_target` spread 1364-2408 MiB over "14 boots of one unchanged config") | **RETRACTED BY ITS OWN AUTHOR.** The 14 boots were >=11 DIFFERENT configs -- the #656 shift was changing `--phase-flip-tp-vector` and others between them -- and R1 pooled them into one distribution and called the variance nondeterminism. Grouped by full post signature: 11 signatures, one with 4 members, and those 4 (spread over 2.5 h, interleaved) are **byte-identical on every post and every card**. A 5th boot on the same argv but a DIFFERENT code tree reproduced it exactly: resident 28436 / 18002 / 16674 MiB. **Given its config the boot's demand is exact and repeatable to the byte** | HANDOFF_605_FLIGHTRECORDER_R2 §1 |
| C605-6 | `weights_target` moves 702 MiB between boots because `--phase-flip-policy auto` picks a layout | **falsified.** It takes exactly 3 values per card and the increments are IDENTICAL on all three cards (+28, +674). A layout re-partition moves weight BETWEEN cards and conserves the total; three cards rising together by the same amounts is a per-rank constant from a changed flag, i.e. between-config variation, not a layout lottery | HANDOFF_605_FLIGHTRECORDER_R2 §2 |
| C605-7 | why no `ledger_<boot>.json` exists on ship boots | assumed a wiring gap or an unset env var (`SGLANG_VRAM_FLIGHT_DIR` is in fact present in the ship env capture) | **structural: the ship pins `--rank-gpu-memory-mib`, which is the PIN PATH, and the pin path skips the planner, so no ledger is ever CONSTRUCTED.** 14 boots of measured marks sat beside zero modelled counterparts and `reconcile.py` had nothing to compare. `ServerArgs.__post_init__` now builds one for the RECORD (discarded, never consulted for a size), gated on the recorder being armed | HANDOFF_605_FLIGHTRECORDER_R2 §3 |
| C605-8 | the 166-276 MiB per-card residual is a hidden tenant (parent/tokenizer context #237, lane pools #400, multimodal second context #403) | **all three excluded BY DIRECT OBSERVATION**: NVML's per-process list shows exactly ONE pid per card on every boot examined. What remains is card `used` that NVML attributes to NO process -- driver-side per-context bookkeeping -- **irreducible with NVML**, bounded at **164-276 MiB**, and the ONLY term that varies between otherwise byte-identical boots | HANDOFF_605_FLIGHTRECORDER_R2 §4 |
| C605-9 | the reconciliation's `model weights (shards)` measurement: the SUM of the target and draft runners' `pre_weight_load -> weights_loaded` reserved deltas (27800/16492/17464 MiB on boot 1464299) | **falsified by the marks themselves.** The process FREES each load before the next: the 5090's `reserved_bytes` falls 21724 -> 8758 between them, so the sum charges memory that was given back, and 27800 MiB exceeds the card's whole process footprint at `boot_complete` (26364 MiB). The basis was also arena-blind under `--phase-flip-spill-depth arena`. Corrected: one EPISODE per `pre_weight_load -> weights_loaded` pair, measured on allocated bytes outside the KV arena, term = the LARGEST episode. Boot 1464299 has THREE episodes per card: 13674/13850/2074, 8325/7719/1897, 9293/7720/1897 | RECONCILE_SECOND_RUN.md §1a |
| C605-10 | `LOAD_TRANSIENT_REFERENCE_MIB = 70`, inherited from window-2026-08-06, and its mapping `("field", allocator_transient_bytes, "weights_loaded")` | **both falsified.** The mapping read the one mark at which the allocator has just handed its peak back and measured 0 on all three cards; the real peaks sit three marks later at 13392/8058/9030 MiB. And the post is **not a constant at all**: over 462-472 recorded boots it spans 0-18486 / 0-9472 / 0-11898 MiB per card. No replacement constant is honest, so the term is REFUSED rather than re-fitted, and the mapping is now the MAXIMUM over the whole boot | RECONCILE_SECOND_RUN.md §1b, §3 |
| C605-11 | the ledger's hardware residual, 664 MiB (5090) / 312 MiB (each 3080), calibrated under fingerprint `a191a0712717` | **25-34% low**, confirmed against 462-472 recorded boots rather than the single boot that first suggested it. Measured bands: **802-902 MiB** (5090) and **360-496 MiB** (both 3080s), charged at the band HIGH. This boot's 886/480/480 sit inside them | RECONCILE_SECOND_RUN.md §3 |
| C605-12 | `reconcile.measured_demand` subtracting the ledger's BUDGETED `kv_pool_mib` from a measured footprint | **wrong basis, and it drove the totals row negative** (-3045 MiB on the 5090). The ledger budgeted 29927 MiB there and the arena actually backed 21130, so the subtraction removed 8797 MiB the process never held. The first run patched around it in prose. Now read from the arena census (`kv_arena_backed_bytes` at `boot_complete`), falling back to the budget only when the marks cannot say | RECONCILE_SECOND_RUN.md §1c |
| C605-13 | `attribute_flight.py reconcile` printing "The ledger names no card whose rank left marks" and exiting 1 on live acceptance boots -- read by two shifts as a statement about the boot | **a caller-side keying defect.** `read_marks` was correctly changed to PID keying (R1 defect 1a) because the ship config runs `--tp-size 1 --pp-size 3` and all three processes file under TP rank 0; the `_reconcile()` caller was never updated and handed the pid dict into `marks_by_rank.get(int(rank))`. Pids (1918126..8) can never equal ranks (0..2), so EVERY card matched nothing and the skip fired for all of them. Fixed by `marks_by_rank_from_pids()`, which re-keys via card uuid -> caller's key -> max rank on the marks, cross-checks `nvml_total_bytes` against the card, and REFUSES on any card left unmatched | RECONCILE_SECOND_RUN.md §7, live boot 1917721-1786622304 |
| C605-14 | "a rank that left no marks is skipped, not invented" (`test_a_rank_that_left_no_marks_is_skipped_not_invented`) | **falsified as a contract.** Silently skipping an unmatched rank is indistinguishable from a total keying mismatch, and that indistinguishability is exactly what hid C605-13. `reconcile()` now REFUSES and names the unmatched ranks | RECONCILE_SECOND_RUN.md §7 |
| C605-15 | `_field_bytes` taking the FIRST mark at a phase | **always the TARGET runner.** A speculative process reaches every per-runner phase at least twice and the draft runner routinely carries the higher level: on live boot 1917721 the hardware residual reads 886 MiB on the target and **896 MiB on the draft**, so first-match under-read the term on every card of every speculative boot. A field term is a LEVEL, so the measurement is now the MAXIMUM across runner partitions (summing would be wrong); process-level phases appear once and are unchanged | RECONCILE_SECOND_RUN.md §7 |
| C605-16 | using the target runner's `weights_loaded -> kv_pool_sized` reserved growth as the measured KV pool | **rejected by measurement, not by argument.** That gap allocates the KV pool AND the mamba/GDN state pool together: on boot 1464299 it grew 7720 MiB while the arena backed 6916, the ~800 MiB difference being the state pool. Using it over-subtracts by exactly the term the ledger books separately. The arena census (`kv_arena_backed_bytes` at `boot_complete`) is the ONLY accepted source; absent it, the demand REFUSES | RECONCILE_SECOND_RUN.md §7 |
| C605-17 | `NCCL communicator buffers = 0` reported as UNMEASURED | **it is a priced zero, not an absent measurement.** The term carries `not_applicable: true` because barlink carries the collectives and no PyNccl communicator is built. Reporting it as UNMEASURED invites a successor to hunt for the boundary, find the one #605 just added, measure 0, and conclude the recorder is broken. It now renders `N/A -- not applicable to this launch (barlink ...)` and is excluded from the UNMEASURED summary | RECONCILE_SECOND_RUN.md §7 |

**MERGE-R7 renamed the eight `C605-*` rows above.** They arrived from
`feat/vram-flightrecorder-605` labelled `C23`-`C30`, and every one of those
eight labels was ALREADY TAKEN in this file by an unrelated contradiction:
`C23`-`C26` are the four `### C<n>` sections further down (the
`SGLANG_UNEVEN_TOKEN_VECTOR` reading, the #695 rank discovery, the #695
exact-size pin, #644's allocator residual), and `C27`-`C30` are the
narrative entries the body already cites 4, 9, 3 and 6 times respectively.
Merging the labels unchanged would have made eight identifiers ambiguous in
the one document whose entire purpose is that a claim can be looked up. The
#605 rows were renamed rather than the incumbents because they are the
arriving side and have no inbound references in this file; their text is
carried verbatim, nothing was dropped or merged, and the source-branch
handoffs carry a pointer to the new labels. Original -> shipped:
`C23`->`C605-1`, `C24`->`C605-2`, `C25`->`C605-3`, `C26`->`C605-4`,
`C27`->`C605-5`, `C28`->`C605-6`, `C29`->`C605-7`, `C30`->`C605-8`.
