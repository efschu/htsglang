...
if [ "${1:-}" = "--selfcheck" ]; then
  ...
  printf '\n%s\n' "$PLANT" >> "$CE"
  snapshot "$CE"
  mutate "$CE" 'def ring_absent_by_design(' 'def ring_absent_by_design_MUTANT('
  restore "$CE"
...
# --- THE danger direction: the defect restored -----------------------------
m "M1 the predicate collapses back to the weight source alone (the defect)" "$CE" \
  '        str(weight_source) == WEIGHT_SOURCE_EXCHANGE
        and str(inject_mode) == weight_exchange.INJECT_AUTHORITATIVE' \
  '        str(weight_source) == WEIGHT_SOURCE_EXCHANGE'
m "M2 the conjunction becomes a disjunction" "$CE" \
  '        and str(inject_mode) == weight_exchange.INJECT_AUTHORITATIVE' \
  '        or str(inject_mode) == weight_exchange.INJECT_AUTHORITATIVE'
m "M3 an unrecognised inject mode counts as authoritative" "$CE" \
  '        and str(inject_mode) == weight_exchange.INJECT_AUTHORITATIVE' \
  '        and str(inject_mode) != weight_exchange.INJECT_SHADOW'
m "M4 the inject arm never reaches the one call site" "$CE" \
